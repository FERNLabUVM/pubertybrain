...
# 2022-08-02 Several fixes:
#            - fix incorrect check for means/modes/medians for latent variables causing
#              mplus.get.estimated_means, mplus.get.estimated_modes, mplus.get.estimated_medians
#              not listed for series with latent variables
#            - fix mplus.get.estimated_medians for latent variables
#            - fix in mplus.plot.sample_proportions and mplus.plot.estimated_probabilities (non-series),
#              yy not defined before used in attr call
#            - flabels and ulabels not declared in mplus.compute.irt.iic (contributed by RNJ)
#            - steps in mplus.plot.irt.icc, mplus.plot.irt.iic and mplus.plot.irt.tic computed using
#              incorrect variance (and mean in tic) after the first class
# 2022-08-02 Add mplus.plot.estimated_medians_with_percentiles.
# 2022-08-15 Add the following functions to get adjusted estimated means:
#              mplus.get.covariates_for_adjusted_estimated_means
#              mplus.compute.adjusted_estimated_means
#              mplus.get.adjusted_estimated_means
#              mplus.plot.adjusted_estimated_means
# 2022-11-01 Fix functions that use var_names from individual_data; needed to crop the trailing
#            spaces of var_names.
# 2022-11-02 Fix mplus.plot.timeseries.observed for CROSSCLASSIFIED.
#            Fix mplus.plot.timeseries.observed and mplus.get.timeseries.data when factor scores
#            are selected.
#            Fix incorrect name of mplus.get.timeseries.idnums in error message.
#            Replace internal codes :1 and :2 with 'mean' and 'median' in all listing of variable names.
# 2023-02-27 Change mplus.plot.qqnorm to mplus.plot.qqplot.
#            Functionality of mplus.plot.qqplot is changed to match QQ plots in the Mplus Editor.
# 2023-02-27 Various functions updated to allow for numeric arguments for variable and process names.
#            These functions include:
#             - mplus.plot.sample_means
#             - mplus.plot.estimated_means
#             - mplus.plot.sample_and_estimated_means
#             - mplus.plot.estimated_modes
#             - mplus.plot.estimated_medians_with_percentiles
#             - mplus.plot.estimated_medians
#             - mplus.plot.adjusted_estimated_means
#             - mplus.plot.estimated_probabilities
#             - mplus.plot.sample_proportions
#             - mplus.plot.sample_proportions_and_estimated_probabilities
# 2023-02-27 Various functions updated to return a data.frame when requested:
#            These functions include:
#             - mplus.plot.sample_means
#             - mplus.plot.estimated_means
#             - mplus.plot.sample_and_estimated_means
...
#             - mplus.plot.sample_proportions
#             - mplus.plot.sample_proportions_and_estimated_probabilities
# 2023-02-27 Add mplus.plot.dropout_means for Dropout means.
# 2023-02-27 Add mplus.plot.individual.values for various individual plots.
# 2023-02-27 Add arguments to various functions for supporting between-level plots.
#            These functions include:
#             - mplus.plot.histogram
#             - mplus.plot.densityplot
#             - mplus.plot.qqplot
#             - mplus.plot.scatterplot
# 2023-02-27 Add time argument for survival plot functions.  Also add discrete
#            argument to handle plots for discrete survival plots.
# 2023-02-27 Add functions for various plots for time series analysis
#            Functions added:
#            - mplus.plot.histogram.timepoints
#            - mplus.plot.time.intervals
# 2023-02-27 Add functions for measurement parameter plots,
#            mplus.plot.measurement.parameter
# 2023-02-27 Add functions for getting/computing between-level information, eg.
#            - mplus.get.estimated.cluster.values
#            - mplus.get.estimated.cluster.variances
#            - mplus.get.values.over.within.cluster
# 2023-02-27 Add supporting functions for used in Mplus Plots User Interface app.
# 2023-11-14 Fix mplus.plot.loop when noci=TRUE
# 2024-01-04 Fix for mplus.compute.individual.estimated.values
# 2024-01-05 Add to mplus.list.variables a listing of variables that have estimated values and residuals.
#            Add to mplus.plot.histogram and mplus.plot.scatterplot a check when estimated or residual
#                is requested but variable does not have estimated values or residuals.
# 2024-05-07 Add all functions for autoregressive curves introduced in Version 8.11.
#            Functions are:  mplus.plot.autoregressive.curve
#                            mplus.list.autoregressive.curve.parameters
#                            mplus.get.autoregressive.estimates
#                            mplus.get.autoregressive.lowerci
#                            mplus.get.autoregressive.upperci
#                            mplus.get.autoregressive.timevalues
# 2024-05-10 Update mplus.plot.timeseries.observed with continuous time
# 2024-05-13 Fixed mplus.plot.histogram.timepoints for time series analysis with TINTERVAL and without TINTERVAL
# 2024-05-14 Update mplus.plot.histogram.timepoints for time series analysis with continuous time
# 2024-08-29 Fix problems in mplus.compute.individual.estimated.values.
# 2024-10-04 Added parallel analysis values to mplus.plot.eigenvalues.
#
# TODO:  Update histogram of subjects per timepoint for continuous time
#
...
mplus.load <- function(file) {
	...
	if ("individual_data" %in% names(gh5)) {
		...
		if (exists("mplus.plot.qqplot",mode="function")) {
			cat(c(" - mplus.plot.qqplot('"),file,"',variable)\n",sep="")
		}
		...
		if (mplus.check.group.attribute(file,"individual_data","timeseries")) {
			if (exists("mplus.plot.timeseries.observed",mode="function")) {
				if (mplus.check.group.attribute(file,'individual_data','cluster')) {
					if (mplus.check.group.attribute(file, 'individual_data', 'continuoustime')) {
						cat(c(" - mplus.plot.timeseries.observed('"),file,"',variable,idnum,binlength)\n",sep="")
					} else {
			...
			if (exists("mplus.plot.histogram.timepoints",mode="function")) {
				if (mplus.check.group.attribute(file, 'individual_data', 'continuoustime')) {
					cat(c(" - mplus.plot.histogram.timepoints('"),file,"',binlength)\n",sep="")
				} else {
					cat(c(" - mplus.plot.histogram.timepoints('"),file,"')\n",sep="")
				}
			}
			if (exists("mplus.plot.time.intervals",mode="function")) {
				if (mplus.check.group.attribute(file,'individual_data','intervals')) {
					cat(c(" - mplus.plot.time.intervals('"),file,"',idnum)\n",sep="")
				}
			}
		}
	}
		...
		for (i in c(1:np)) {
			...
			if (series_type == 1) {
				...
				if (em_ind > 0 && exists("mplus.plot.estimated_means",mode="function")) {
					...
					if ("matrix_data" %in% names(gh5) && "individual_data" %in% names(gh5)) {
						nx <- as.integer(mplus.get.group.attribute(file, "individual_data", "n_x"))
						if (nx > 0) {
							if (exists("mplus.plot.adjusted_estimated_means",mode="function")) {
								cstr2 <- paste(c(" - mplus.plot.adjusted_estimated_means('"),file,"',dfcovariates,'",cstr,"')\n",sep="")
								cat(cstr2)
							}
						}
					}
				...
				if (em_ind > 0 && exists("mplus.plot.estimated_medians",mode="function")) {
					...
					cat(cstr2)
					em_ind <- pmatch("y_quantiles",values,nomatch=0)
					if (em_ind > 0 && exists("mplus.plot.estimated_medians_with_percentiles",mode="function")) {
						cstr2 <- paste(c(" - mplus.plot.estimated_medians_with_percentiles('"),file,"','",cstr,"')\n",sep="")
						cat(cstr2)
					}
				}
				...
				if (em_ind > 0 && exists("mplus.plot.estimated_medians",mode="function")) {
					cstr2 <- paste(c(" - mplus.plot.estimated_medians('"),file,"','",cstr,"')\n",sep="")
					cat(cstr2)
					em_ind <- pmatch("latent_quantiles",values,nomatch=0)
					if (em_ind > 0 && exists("mplus.plot.estimated_medians_with_percentiles",mode="function")) {
						cstr2 <- paste(c(" - mplus.plot.estimated_medians_with_percentiles('"),file,"','",cstr,"')\n",sep="")
						cat(cstr2)
					}
	...
	if ("drift_curve_data" %in% names(gh5)) {
		if (exists("mplus.list.autoregressive.curve.parameters",mode="function")) {
			cat(c(" - mplus.list.autoregressive.curve.parameters('"),file,"')\n",sep="")
		}
		if (exists("mplus.plot.autoregressive.curve",mode="function")) {
			cat(c(" - mplus.plot.autoregressive.curve('"),file,"',label,showgrid)\n",sep="")
		}
	}
	...
	if ("drift_curve_data" %in% names(gh5)) {
		if (exists("mplus.get.autoregressive.estimates",mode="function")) {
			cat(c(" - mplus.get.autoregressive.estimates('"),file,"',label)\n",sep="")
		}
		if (exists("mplus.get.autoregressive.lowerci",mode="function")) {
			cat(c(" - mplus.get.autoregressive.lowerci('"),file,"',label)\n",sep="")
		}
		if (exists("mplus.get.autoregressive.upperci",mode="function")) {
			cat(c(" - mplus.get.autoregressive.upperci('"),file,"',label)\n",sep="")
		}
		if (exists("mplus.get.autoregressive.timevalues",mode="function")) {
			cat(c(" - mplus.get.autoregressive.timevalues('"),file,"')\n",sep="")
		}
	}
			...
			if (prop[1] == 1) {
				...
				if (em_ind > 0 && exists("mplus.get.estimated_means",mode="function")) {
					...
					if ("matrix_data" %in% names(gh5) && "individual_data" %in% names(gh5)) {
						nx <- as.integer(mplus.get.group.attribute(file, "individual_data", "n_x"))
						if (nx > 0) {
							if (exists("mplus.get.covariates.for.adjusted_estimated_means",mode="function")) {
								cstr2 <- paste(c(" - mplus.get.covariates.for.adjusted_estimated_means('"),file,"',classidx)\n",sep="")
								cat(cstr2)
							}
							if (exists("mplus.get.adjusted_estimated_means",mode="function")) {
								cstr2 <- paste(c(" - mplus.get.adjusted_estimated_means('"),file,"',dfcovariates,'",cstr,"')\n",sep="")
								cat(cstr2)
							}
						}
					}
				...
				if (em_ind > 0 && exists("mplus.get.estimated_medians",mode="function")) {
					...
					cat(cstr2)
					em_ind <- pmatch("y_quantiles",values,nomatch=0)
					if (em_ind > 0 && exists("mplus.get.estimated_medians_with_percentiles",mode="function")) {
						cstr2 <- paste(c(" - mplus.get.estimated_medians_with_percentiles('"),file,"','",cstr,"')\n",sep="")
						cat(cstr2)
					}
				}
			} else if (prop[1] == 2) {
...
#
mplus.list.processes <- function(file,quiet=FALSE,ptype) {
	if (missing(file)) {
	...
	if (missing(ptype)) {
		...
		selectprocess <- vector()
		for (i in c(1:length(allpnames))) {
			cstr2 <- paste(c("process_data"),"/",allpnames[i],"", sep="")
			prop <- mplus.get.group.attribute(file, cstr2, 'properties')
			if (ptype == prop[1]) {
				selectprocess <- append(selectprocess, allpnames[i])
			}
		}
...
#
mplus.plot.estimated_means <-function(file,procstr='process1',ptype='o',getdf=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (is.numeric(procstr)) {
		procstr <- paste("process",procstr,sep="")
	}
	allpnames <- attr(gh5$process_data,"names")
	...
	df <- data.frame()
	for (i in c(1:dims[2])) {
		df <- rbind(df, xx[,i], yy[,i])
		lines(xx[,i],yy[,i],type=ptype,pch=symb[i],col=colors[i])
	}

	rnames <- array(0,2*c(dims[2]))
	cnames <- seq(1:c(dims[1]))
	...
	for (i in c(1:dims[2])) {
		...
		rnames[2*i-1] <- sprintf("Time scores, Class %d", i)
		rnames[2*i] <- sprintf("Estimated means, Class %d", i)
		lty[i] = 1
		lwd[i] = 2.5
		lcol[i] <- colors[i]
	}
...
##########################################################################
#
# mplus.plot.adjusted_estimated_means - plot adjusted estimated means for the quoted process
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	dfcovariates - data frame containing the values of covariates to use in computing adjusted estimated means
#		Use mplus.get.covariates_for_adjusted_means function to get a template data frame.
#		The column names of the data frame will be used to label the lines in the legend.
#		If the data frame contains a Name column containing the names of the covariates, this column will be ignored.
#	procstr - the quoted name of a series
#
# eg. mplus.plot.adjusted_estimated_means('ex8.1.gh5',dfx,'process1')
#
mplus.plot.adjusted_estimated_means <-function(file,dfcovariates,procstr='process1',ptype='o',getdf=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	series_type <- prop[1]
	if ( ! (series_type == 1 || series_type == 2) ) {
		cstr <- paste("- process does not have adjusted estimated means:",procstr,"\n\n")
		stop(cstr)
	}

	if (missing(dfcovariates)) {
		cstr <- sprintf("A data frame of values for the covariates must be specified.")
		stop(cstr)
	}
	if (!is.data.frame(dfcovariates)) {
		cstr <- sprintf("A data frame of values for the covariates must be specified.  Argument is not a data frame.")
		stop(cstr)
	}
	namecol <- length(which(colnames(dfcovariates) == 'Name'))
	nsets <- ncol(dfcovariates) - namecol

	# set up the array for the estimated means
	dims <- attr(proc$time_scores,"dim")
	xx <- array(0, c(dims[1],dims[2]*nsets))
	yy <- mplus.get.adjusted_estimated_means(file,procstr,dfcovariates=dfcovariates)
	...
	for (i in c(1:dims[2])) {
		for (s in c(1:nsets)) {
			k <- k + 1
			xx[,k] <- proc$time_scores[,i]
		}
	}

	# plot the means
	cstr <- paste("Adjusted estimated means for",procstr)
	plot(xx,yy,xlab="",ylab="",main=cstr,type='n')
	symb <- array(c(21,22,23,24,25),c(dims[1]))
	colors <- rainbow(dims[2]*nsets)
	...
	for (i in c(1:(dims[2]))) {
		for (j in c(1:nsets)) {
			k <- k + 1
			df <- rbind(df, xx[,k], yy[,k])
			lines(xx[,k],yy[,k],type=ptype,pch=symb[j],col=colors[i])
		}
	}

	rnames <- array(0,(nsets+1)*c(dims[2]))
	cnames <- seq(1:c(dims[1]))

	ldesc <- array(0,c(nsets*dims[2]))
	lty <- array(0,c(nsets*dims[2]))
	lwd <- array(0,c(nsets*dims[2]))
	lcol <- array(0,c(nsets*dims[2]))
	lsymb <- array(0,c(nsets*dims[2]))
	k <- 0
	for (i in c(1:dims[2])) {
		rnames[(i-1)*nsets+1] <- sprintf("Time scores, Class %d", i)

		for (j in c(1:nsets)) {
			k <- k + 1
			ldesc[k] <- sprintf("%s, Class %d", colnames(dfcovariates)[namecol+j], i)

			rnames[(i-1)*nsets+j+1] <- sprintf("%s, Class %d", colnames(dfcovariates)[namecol+j], i)
...
#
mplus.plot.estimated_modes <-function(file,procstr='process1',ptype='o',getdf=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	for (i in c(1:dims[2])) {
		ldesc[i] <- sprintf("Class %d", i)
		rnames[2*i-1] <- sprintf("Time scores, Class %d", i)
		rnames[2*i] <- sprintf("Estimated modes, Class %d", i)
		lty[i] = 1
		lwd[i] = 2.5
		lcol[i] <- colors[i]
	}
...
#
mplus.plot.estimated_medians <-function(file,procstr='process1',ptype='o',getdf=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	for (i in c(1:dims[2])) {
		df <- rbind(df, xx[,i], yy[,i])
		lines(xx[,i],yy[,i],type=ptype,pch=symb[1],col=colors[i])
	}
	...
	for (i in c(1:dims[2])) {
		ldesc[i] <- sprintf("Class %d", i)
		rnames[2*i-1] <- sprintf("Time scores, Class %d", i)
		rnames[2*i] <- sprintf("Estimated medians, Class %d", i)
		lty[i] = 1
		lwd[i] = 2.5
		lcol[i] <- colors[i]
	}
...
##########################################################################
#
# mplus.plot.estimated_medians_with_percentiles - plot estimated medians and quantiles for the quoted process
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	procstr - the quoted name of a series
#
# eg. mplus.plot.estimated_medians_with_percentiles('ex8.1.gh5','process1')
#
mplus.plot.estimated_medians_with_percentiles <-function(file,procstr='process1',ptype='l',getdf=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	# set up the array for the estimated means
	dims <- attr(proc$time_scores,"dim")
	xx <- array(0, c(dims[1],7*dims[2]))
	yy <- array(0, c(dims[1],7*dims[2]))
	perc <- mplus.get.estimated_medians_with_percentiles(file,procstr)

	for (i in c(1:dims[2])) {
		# set the time scores and pick up each of the 7 percentile
		for (j in c(1:7)) {
			xx[,7*(i-1)+j] <- proc$time_scores[,i]
			yy[,7*(i-1)+j] <- perc[,j,i]
		}
	}

	# plot the means
	cstr <- paste("Estimated medians and percentiles for",procstr)
	plot(xx,yy,xlab="",ylab="",main=cstr,type='n')
	colors <- rainbow(dims[2])
	thickness <- c(1,1,1,2,1,1,1)

	df <- data.frame()
	for (i in c(1:dims[2])) {
		df <- rbind(df, xx[,7*(i-1)+1])
		for (j in c(1:7)) {
			df <- rbind(df, yy[,7*(i-1)+j])
			lines(xx[,7*(i-1)+j],yy[,7*(i-1)+j],type=ptype,col=colors[i],lwd=thickness[j])
		}
	}

	rnames <- array(0,8*c(dims[2]))
	cnames <- seq(1:c(dims[1]))
	...
	for (i in c(1:dims[2])) {
		...
		rnames[8*i-7] <- sprintf("Time scores, Class %d", i)
		rnames[8*i-6] <- sprintf("Lower 5%%, Class %d", i)
		rnames[8*i-5] <- sprintf("Lower 10%%, Class %d", i)
		rnames[8*i-4] <- sprintf("Lower 25%%, Class %d", i)
		rnames[8*i-3] <- sprintf("Median, Class %d", i)
		rnames[8*i-2] <- sprintf("Upper 25%%, Class %d", i)
		rnames[8*i-1] <- sprintf("Upper 10%%, Class %d", i)
		rnames[8*i] <- sprintf("Upper 5%%, Class %d", i)

		lty[i] = 1
		lwd[i] = 2.5
		lcol[i] <- colors[i]
	}
	legend('bottomright',col=lcol,ldesc,lty=lty,lwd=lwd)
...
#	procstr - the quoted name of a series
#
# eg. mplus.plot.sample_means('ex6.1.gh5','process1',getdf=FALSE)
#
mplus.plot.sample_means <-function(file,procstr='process1',ptype='o',getdf=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	for (i in c(1:dims[2])) {
		ldesc[i] <- sprintf("Class %d", i)
		rnames[2*i-1] <- sprintf("Time scores, Class %d", i)
		rnames[2*i] <- sprintf("Sample means, Class %d", i)
		lty[i] = 1
		lwd[i] = 2.5
		lcol[i] <- colors[i]
	}
...
#
mplus.plot.sample_and_estimated_means <-function(file,procstr='process1',ptype='o',getdf=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	for (i in c(1:dims[2])) {
		...
		rnames[3*i-2] <- sprintf("Time scores, Class %d", i)
		rnames[3*i-1] <- sprintf("Sample means, Class %d", i)
		rnames[3*i] <- sprintf("Estimated means, Class %d", i)
...
##########################################################################
#
# mplus.plot.dropout_means - plot dropout means for the quoted process
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	procstr - the quoted name of a series
#
# eg. mplus.plot.dropout_means('ex8.1.gh5','process1')
#
mplus.plot.dropout_means <-function(file,procstr='process1',ptype='o',getdf=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	series_type <- prop[1]
	if (series_type != 1) {
		cstr <- paste("- process does not have dropout means:",procstr,"\n\n")
		stop(cstr)
	}

	# get the time scores
	dims <- attr(proc$time_scores,"dim")
	xx <- array(0,c(dims[1],dims[2],3))

	xx[,,1] <- proc$time_scores
	xx[,,2] <- proc$time_scores
	xx[,,3] <- proc$time_scores

	# set up the array for the estimated means
	yy <- mplus.get.dropout_means(file,procstr) # third column is type: dropout, nondropout, overall

	# plot the means
	cstr <- paste("Dropout means for",procstr)
	plot(xx,yy,xlab="",ylab="",main=cstr,type='n')
	symb <- array(c(21,22,23,24,25),c(dims[1]))
	colors <- rainbow(dims[2])
	...
	for (i in c(1:dims[2])) {
		# Overall
		lines(xx[,i,3],yy[,i,3],type=ptype,pch=1,col=colors[i])

		# Non-dropouts
		points(xx[,i,2], yy[,i,2],pch=2, col=colors[i])

		# Dropouts
		points(xx[,i,1], yy[,i,1],pch=0, col=colors[i])

		df <- rbind(df, xx[,i,1], yy[,i,3], yy[,i,2], yy[,i,1])
	}

	rnames <- array(0,4*c(dims[2]))
	cnames <- seq(1:c(dims[1]))

	ldesc <- array(0,3*c(dims[2]))
	lty <- array(0,3*c(dims[2]))
	lwd <- array(0,c(dims[2]))
	lpch <- array(0,3*c(dims[2]))
	lcol <- array(0,c(dims[2]))
	for (i in c(1:dims[2])) {
		if (dims[2] > 1) {
			ldesc[3*i-2] <- sprintf("Overall, Class %d")
			ldesc[3*i-1] <- sprintf("Non-dropouts, Class %d", i)
			ldesc[3*i] <- sprintf("Dropouts, Class %d", i)
			rnames[4*i-3] <- sprintf("Time scores, Class %d", i)
			rnames[4*i-2] <- sprintf("Overall, Class %d", i)
			rnames[4*i-1] <- sprintf("Non-dropouts, Class %d", i)
			rnames[4*i] <- sprintf("Dropouts, Class %d", i)
		} else {
			ldesc[3*i-2] <- sprintf("Overall")
			ldesc[3*i-1] <- sprintf("Non-dropouts")
			ldesc[3*i] <- sprintf("Dropouts")
			rnames[4*i-3] <- sprintf("Time scores")
			rnames[4*i-2] <- sprintf("Overall")
			rnames[4*i-1] <- sprintf("Non-dropouts")
			rnames[4*i] <- sprintf("Dropouts")
		}
		lty[3*i-2] = 1
		lty[3*i-1] = 0
		lty[3*i] = 0
	...
	legend('bottomright',col=lcol,pch=lpch,ldesc,lty=lty,lwd=lwd)
...
#
# eg. mplus.plot.estimated_probabilities('ex8.4.gh5','process1',1,1,quietly=FALSE)
#
mplus.plot.estimated_probabilities <- function(file,var=1,series=FALSE,cat1=1,cat2=1,lposition='top',ptype='o') {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (series) {
		...
		if (is.numeric(var)) {
			if (var < 1 || var > length(catvars)) {
				cstr <- paste("- variable index is out of range, should be between 1 and ",length(catvars),":\n")
				stop(cstr)
			}
			cat_index <- var
			var <- catvars[cat_index]
		} else {
			var <- toupper(var)
...
#
mplus.plot.sample_proportions <-function(file,var=1,series=FALSE,cat1=1,cat2=1,lposition='top',ptype='o') {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
#
mplus.plot.sample_proportions_and_estimated_probabilities <-function(file,var=1,series=FALSE,cat1=1,cat2=1,lposition='top',ptype='o') {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
mplus.get.estimated_means <-function(file,procstr='process1',classidx=0) {
	...
	if (series_type == 1) {
		if (classidx == 0) {
			...
			for (j in c(1:dims[1])) {
				yy[j,1] <- gh5$means_and_variances_data$y_estimated_means$values[var_indices[j],classidx]
			}
		}
	}
	...
	if (series_type == 2) {
		if (classidx == 0) {
			...
			for (j in c(1:dims[1])) {
				yy[j,1] <- gh5$means_and_variances_data$latent_estimated_means$values[var_indices[j],classidx]
			}
		}
	}
...
mplus.get.sample_means <- function(file,procstr='process1',classidx=0) {
	...
	if (classidx == 0) {
		...
		for (j in c(1:dims[1])) {
			yy[j,1] <- gh5$means_and_variances_data$y_observed_means$values[var_indices[j],classidx]
		}
	}
...
##########################################################################
#
# mplus.get.covariates.for.adjusted_estimated_means - get dataframe of covariates
#
# arguments:
#	file - the quoted name of an existing GH5 file, required
#	classidx - the class index, not required - 0 for template only.  Default to 0.
#
# eg. mplus.get.covariates.for.adjusted_estimated_means('ex8.1.gh5')
#
mplus.get.covariates.for.adjusted_estimated_means <- function(file,classidx=0) {
	if (missing(file)) {
		stop(" - name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if ( !("matrix_data" %in% names(gh5)) ) {
		stop("Matrix data is required.\n\nUse TYPE=PLOT2.")
	}

	if (!is.null(setnames)) {
		if (length(setnames) != sets) {
			cstr <- paste("- size of setnames does not match the number of sets argument:",length(setnames),"and",sets)
			stop(cstr)
		}
	}

	#	properties[1] - number of p
	#	properties[2] - number of f
	#	properties[3] - number of x
	#	properties[4] - number of thresholds
	#	properties[5] - number of classes
	props <- mplus.get.group.attribute(file,'matrix_data','properties')

	num_fx <- as.integer(props[2])
	num_x <- as.integer(props[3])
	num_f <- num_fx - num_x

	labels <- mplus.get.group.attribute(file,'matrix_data', 'alphalabels')

	if (missing(classidx)) {
		df <- data.frame(matrix(NA,nrow=num_x))
	} else {
		df <- data.frame(gh5$matrix_data$alpha[(num_f+1):(num_f+num_x),classidx])
	}
	if (is.null(setnames)) {
		colnames(df) <- c("Value")
	} else {
		colnames(df) <- setnames[1]
	}
	rownames(df) <- labels[(num_f+1):(num_f+num_x)]

	if (sets > 1) {
		for (i in c(2:sets)) {
			if (missing(classidx)) {
				df1 <- data.frame(matrix(NA,nrow=num_x))
			} else {
				df1 <- data.frame(gh5$matrix_data$alpha[(num_f+1):(num_f+num_x),classidx])
			}
			if (is.null(setnames)) {
				colnames(df1) <- paste("Value",i,sep="")
			} else {
				colnames(df1) <- setnames[i]
			}
			rownames(df1) <- labels[(num_f+1):(num_f+num_x)]
			df <- cbind(df,df1)
		}
	}
...
##########################################################################
#
# mplus.compute.adjusted_estimated_means - compute the estimated means based on given data frame of covariate values
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	group - the group or class index
#	dfcovariates - data frame containing the values of covariates to use in computing adjusted estimated means
#		Use mplus.get.covariates_for_adjusted_means function to get a template data frame.
#		The column names of the data frame will be used to label the lines in the legend.
#		If the data frame contains a Name column containing the names of the covariates, this column will be ignored.
#	column - use specific column in the data frame
#		If missing, the first column not the "Name" column will be used.
#	islatent - TRUE if computing means for the latent variables, FALSE for the means of observed variables.  Default is FALSE.
#
# eg. mplus.compute.adjusted_estimated_means('ex8.1.gh5',dfx,)
#
mplus.compute.adjusted_estimated_means <- function(file,group,dfcovariates,column,islatent=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (length(dfcovariates[[1]]) != num_x) {
		cstr <- sprintf("The length of the covariates vector should be %d.\nFound: %d", num_x, length(covariates$Use))
		stop(cstr)
	}
	if (missing(column)) {
		column <- which(colnames(dfcovariates) != 'Name')[1]
	} else {
		if (column > ncol(dfcovariates)) {
			cstr <- sprintf("The column index is bigger than the number of columns (%d) in the data frame.\nGiven: %d", ncol(dfcovariates), column)
			stop(cstr)
		} else if (!(column %in% which(colnames(dfcovariates) != 'Name'))) {
			cstr <- sprintf("The column index should be a data frame column that is not 'Name'.")
			stop(cstr)
		}
	}

	xlabels <- mplus.get.group.attribute(file,'matrix_data', 'alphalabels')[(num_f+1):(num_f+num_x)]

	alpha <- gh5$matrix_data$alpha[,group]
	for (i in 1:length(dfcovariates[[column]])) {
		if (!is.na(dfcovariates[[column]][i])) {
			alpha[num_f+i] <- dfcovariates[[column]][i]
		}
	}

	if (islatent) {
		means <- t(gh5$matrix_data$inv_ibeta[,,group]) %*% alpha
	} else {
		means <- gh5$matrix_data$nu[,group] + t(gh5$matrix_data$lambda[,,group]) %*% t(gh5$matrix_data$inv_ibeta[,,group]) %*% alpha
	}
	t(means)
}
...
##########################################################################
#
# mplus.get.adjusted_estimated_means - get adjusted estimated means for the quoted process
#
# arguments:
#	file - the quoted name of an existing GH5 file, required
#	dfcovariates - data frame containing the values of covariates to use in computing adjusted estimated means
...
#	classidx - the class index, not required - 0 for all classes.  Default to 0.
#
# eg. mplus.get.adjusted_estimated_means('ex8.1.gh5',dfx,'process1',3)
#
mplus.get.adjusted_estimated_means <- function(file,dfcovariates,procstr='process1',classidx=0) {
	if (missing(file)) {
		stop(" - name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	nsets <- ncol(dfcovariates) - length(which(colnames(dfcovariates) == 'Name'))

	# set up the array for the estimated means
	dims <- attr(proc$time_scores,"dim")
	# if all classes, dimension it by number of classes.  Otherwise, just dimension by 1.
	if (classidx == 0) {
		yy <- array(0, c(dims[1],dims[2]*nsets))
	} else {
		# check that the classidx is within range.
		if (classidx < 0 || classidx > dims[2]) {
			cstr <- paste("- classidx is out of range, 1 to ",dims[2],": ",classidx,"\n\n")
	...
	if (classidx == 0) {
		for (s in c(1:nsets)) {
			column <- which(colnames(dfcovariates) != 'Name')[s]

			for (i in c(1:dims[2])) {
				if (series_type == 2) {
					means <- mplus.compute.adjusted_estimated_means(file,i,dfcovariates,column,TRUE)
				} else {
					means <- mplus.compute.adjusted_estimated_means(file,i,dfcovariates,column)
				}
				for (j in c(1:dims[1])) {
					yy[j,s+(i-1)*nsets] <- means[var_indices[j]]
				}
			}
		}
	} else {
		for (s in c(1:nsets)) {
			column <- which(colnames(dfcovariates) != 'Name')[s]
			if (islatent) {
				means <- mplus.compute.adjusted_estimated_means(file,classidx,dfcovariates,column,TRUE)
			} else {
				means <- mplus.compute.adjusted_estimated_means(file,classidx,dfcovariates,column)
			}
			for (j in c(1:dims[1])) {
				yy[j,s] <- means[var_indices[j]]
			}
		}
	}

	# return the adjusted means
	return(yy)
}
...
mplus.get.estimated_modes <-function(file,procstr='process1',classidx=0) {
	...
	if (series_type == 1) {
		...
		mean_vars <- mplus.get.group.attribute(file,'means_and_variances_data/latent_estimated_modes','variables')
	}
	var_indices <- pmatch(var_names, mean_vars, nomatch=0)
	...
	if (series_type == 1) {
		if (classidx == 0) {
			...
			for (j in c(1:dims[1])) {
				yy[j,1] <- gh5$means_and_variances_data$y_estimated_modes$values[var_indices[j],classidx]
			}
		}
	}
	...
	if (series_type == 2) {
		if (classidx == 0) {
			for (i in c(1:dims[2])) {
				for (j in c(1:dims[1])) {
					yy[j,i] <- gh5$means_and_variances_data$latent_estimated_modes$values[var_indices[j],i]
				}
			}
		} else {
			for (j in c(1:dims[1])) {
				yy[j,1] <- gh5$means_and_variances_data$latent_estimated_modes$values[var_indices[j],classidx]
			}
		}
	}
...
mplus.get.estimated_medians <-function(file,procstr='process1',classidx=0) {
	...
	if (series_type == 1) {
		...
		mean_vars <- mplus.get.group.attribute(file,'means_and_variances_data/latent_estimated_medians','variables')
	}
	var_indices <- pmatch(var_names, mean_vars, nomatch=0)
	...
	if (series_type == 1) {
		if (classidx == 0) {
			...
			for (j in c(1:dims[1])) {
				yy[j,1] <- gh5$means_and_variances_data$y_estimated_medians$values[var_indices[j],classidx]
			}
		}
	}
	...
	if (series_type == 2) {
		if (classidx == 0) {
			for (i in c(1:dims[2])) {
				for (j in c(1:dims[1])) {
					yy[j,i] <- gh5$means_and_variances_data$latent_estimated_medians$values[var_indices[j],i]
				}
			}
		} else {
			for (j in c(1:dims[1])) {
				yy[j,1] <- gh5$means_and_variances_data$latent_estimated_medians$values[var_indices[j],classidx]
			}
		}
	}
...
##########################################################################
#
# mplus.get.estimated_medians_with_percentiles - plot estimated medians with percentiles for the quoted process
#
# arguments:
#	file - the quoted name of an existing GH5 file, required
#	procstr - the quoted name of a series, not required.  Defaults to 'process1' (the first process)
#	classidx - the class index, not required - 0 for all classes.  Default to 0.
#
# eg. mplus.get.estimated_medians_with_percentiles('ex8.1.gh5','process1',3)
#
mplus.get.estimated_medians_with_percentiles <-function(file,procstr='process1',classidx=0) {
	if (missing(file)) {
		stop(" - name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (classidx == 0) {
		yy <- array(0, c(dims[1],dims[2]))
		quant5 <- array(0, c(dims[1],dims[2]))
		quant10 <- array(0, c(dims[1],dims[2]))
		quant25 <- array(0, c(dims[1],dims[2]))
		quant75 <- array(0, c(dims[1],dims[2]))
		quant90 <- array(0, c(dims[1],dims[2]))
		quant95 <- array(0, c(dims[1],dims[2]))
	} else {
		# check that the classidx is within range.
		if (classidx < 0 || classidx > dims[2]) {
			cstr <- paste("- classidx is out of range, 1 to ",dims[2],": ",classidx,"\n\n")
		...
		yy <- array(0, c(dims[1],1))
		quant5 <- array(0, c(dims[1],1))
		quant10 <- array(0, c(dims[1],1))
		quant25 <- array(0, c(dims[1],1))
		quant75 <- array(0, c(dims[1],1))
		quant90 <- array(0, c(dims[1],1))
		quant95 <- array(0, c(dims[1],1))
	}
	...
	# type 1 is estimated medians and quantiles for observed variables
	if (series_type == 1) {
		if (classidx == 0) {
			for (i in c(1:dims[2])) {
				for (j in c(1:dims[1])) {
					yy[j,i] <- gh5$means_and_variances_data$y_estimated_medians$values[var_indices[j],i]
					quant5[j,i] <- gh5$means_and_variances_data$y_quantiles$values[var_indices[j],1,i]
					quant10[j,i] <- gh5$means_and_variances_data$y_quantiles$values[var_indices[j],2,i]
					quant25[j,i] <- gh5$means_and_variances_data$y_quantiles$values[var_indices[j],3,i]
					quant75[j,i] <- gh5$means_and_variances_data$y_quantiles$values[var_indices[j],4,i]
					quant90[j,i] <- gh5$means_and_variances_data$y_quantiles$values[var_indices[j],5,i]
					quant95[j,i] <- gh5$means_and_variances_data$y_quantiles$values[var_indices[j],6,i]
				}
			}
		} else {
			for (j in c(1:dims[1])) {
				yy[j,] <- gh5$means_and_variances_data$y_estimated_medians$values[var_indices[j],classidx]
				quant5[j,] <- gh5$means_and_variances_data$y_quantiles$values[var_indices[j],1,classidx]
				quant10[j,] <- gh5$means_and_variances_data$y_quantiles$values[var_indices[j],2,classidx]
				quant25[j,] <- gh5$means_and_variances_data$y_quantiles$values[var_indices[j],3,classidx]
				quant75[j,] <- gh5$means_and_variances_data$y_quantiles$values[var_indices[j],4,classidx]
				quant90[j,] <- gh5$means_and_variances_data$y_quantiles$values[var_indices[j],5,classidx]
				quant95[j,] <- gh5$means_and_variances_data$y_quantiles$values[var_indices[j],6,classidx]
			}
		}
	}
	...
	if (series_type == 2) {
		if (classidx == 0) {
			for (i in c(1:dims[2])) {
				for (j in c(1:dims[1])) {
					yy[j,i] <- gh5$means_and_variances_data$latent_estimated_medians$values[var_indices[j],i]
					quant5[j,i] <- gh5$means_and_variances_data$latent_quantiles$values[var_indices[j],1,i]
					quant10[j,i] <- gh5$means_and_variances_data$latent_quantiles$values[var_indices[j],2,i]
					quant25[j,i] <- gh5$means_and_variances_data$latent_quantiles$values[var_indices[j],3,i]
					quant75[j,i] <- gh5$means_and_variances_data$latent_quantiles$values[var_indices[j],4,i]
					quant90[j,i] <- gh5$means_and_variances_data$latent_quantiles$values[var_indices[j],5,i]
					quant95[j,i] <- gh5$means_and_variances_data$latent_quantiles$values[var_indices[j],6,i]
				}
			}
		} else {
			for (j in c(1:dims[1])) {
				yy[j,] <- gh5$means_and_variances_data$latent_estimated_medians$values[var_indices[j],classidx]
				quant5[j,] <- gh5$means_and_variances_data$latent_quantiles$values[var_indices[j],1,classidx]
				quant10[j,] <- gh5$means_and_variances_data$latent_quantiles$values[var_indices[j],2,classidx]
				quant25[j,] <- gh5$means_and_variances_data$latent_quantiles$values[var_indices[j],3,classidx]
				quant75[j,] <- gh5$means_and_variances_data$latent_quantiles$values[var_indices[j],4,classidx]
				quant90[j,] <- gh5$means_and_variances_data$latent_quantiles$values[var_indices[j],5,classidx]
				quant95[j,] <- gh5$means_and_variances_data$latent_quantiles$values[var_indices[j],6,classidx]
			}
		}
	}

	if (classidx == 0) {
		allvalues <- array(0, c(dims[1],7,dims[2]))
	} else {
		allvalues <- array(0, c(dims[1],7,1))
	}
	allvalues[,1,] <- quant5
	allvalues[,2,] <- quant10
	allvalues[,3,] <- quant25
...
##########################################################################
#
# mplus.get.dropout_means - get dropout means for the quoted process
#
# arguments:
#	file - the quoted name of an existing GH5 file, required
#	procstr - the quoted name of a series, not required.  Defaults to 'process1' (the first process)
#	classidx - the class index, not required - 0 for all classes.  Default to 0.
#
# eg. mplus.get.dropout_means('ex8.1.gh5','process1',3)
#
mplus.get.dropout_means <-function(file,procstr='process1',classidx=0) {
	if (missing(file)) {
		stop(" - name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (classidx == 0) {
		...
		yy <- array(0, c(dims[1],1,3))
	}

	# get the indices of variables in the series
	var_names <- mplus.get.group.attribute(file,cstr2,'var_names')

	mean_vars <- mplus.get.group.attribute(file,'means_and_variances_data/dropout_means','variables')
	var_indices <- pmatch(var_names, mean_vars, nomatch=0)

	if (classidx == 0) {
		for (i in c(1:dims[2])) {
			for (j in c(1:dims[1])) {
				yy[j,i,] <- gh5$means_and_variances_data$dropout_means$values[var_indices[j],,i]
			}
		}
	} else {
		for (j in c(1:dims[1])) {
			yy[j,1,] <- gh5$means_and_variances_data$dropout_means$values[var_indices[j],,classidx]
		}
	}
	yy[yy == 999] <- NA
...
##########################################################################
#
# mplus.plot.individual.values - plot observed individual values for the quoted process
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	procstr - the quoted name of a series
#
# eg. mplus.plot.individual.values('ex8.1.gh5','process1')
#
mplus.plot.individual.values <-function(file,procstr='process1',start=1,count=10,curve=0,estimated=FALSE,showmean=FALSE,classidx=1,getdf=FALSE,showadjusted=FALSE,dfcovariates=NULL) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	# check that the series exists
	if (!("process_data" %in% names(gh5))) {
		stop("- requires series information\n\nUse the SERIES option in Mplus to specify series information for processes\nwith observed individual values.\n")
	}
	...
	mean_vars <- mplus.get.group.attribute(file, cstr2, 'var_names')
	mean_vars <- gsub("(^\\s+|\\s+$)", "", mean_vars, perl=TRUE)
	mean_vars <- gsub(":1", ", mean", mean_vars, perl=FALSE)
	mean_vars <- gsub(":2", ", median", mean_vars, perl=FALSE)
	mean_vars <- toupper(mean_vars)

	series_type <- prop[1]
	if (series_type != 1) {
		cstr <- paste("- process does not have observed individual values:",procstr,"\n\n")
		stop(cstr)
	}
	...
	if (!is.null(classidx)) {
		if (is.character(classidx)) {
			classstr <- classidx
			classlist <- mplus.get.model_group_labels(file)
			classlist <- gsub("(^\\s+|\\s+$)", "", classlist, perl=TRUE)
			classidx <- pmatch(classstr, classlist, nomatch=0)
		}
	}

	if (showadjusted) {
		showmean <- TRUE
		if (is.null(dfcovariates)) {
			cstr <- paste(" - argument dfcovariates must be given when argument showadjusted is TRUE.\n",
			"Use the function mplus.get.covariates.for.adjusted.estimated.means to get a data frame for the covariates.\n",
			"Edit the given data frame with your own values.")
			stop(cstr)
		}
		namecol <- length(which(colnames(dfcovariates) == 'Name'))
		nsets <- ncol(dfcovariates) - namecol
	...
	if (showmean) {
		if (is.null(classidx) || classidx == 0) {
			yy <- mplus.get.data(file,mean_vars)
		} else {
			# check that the classidx is within range.
			if (classidx < 0 || classidx > dims[2]) {
				cstr <- paste("- classidx is out of range, 1 to ",dims[2],": ",classidx,"\n\n")
			...
			if (estimated) {
				...
				var_indices <- pmatch(var_names, mean_vars, nomatch=0)
				means <- mplus.compute.individual.estimated.values(file,class=classidx)
				yy <- t(means[,var_indices])
				curve <- 0
			} else {
				yy <- mplus.get.data(file,mean_vars,class=classidx)
			}
		}
		dimsy <- attr(yy, 'dim')
		# get the time scores
		...
		if (estimated) {
			...
			var_names <- toupper(var_names)
			var_indices <- pmatch(var_names, mean_vars, nomatch=0)
			means <- mplus.compute.individual.estimated.values(file,obs=start:(start+count-1))
			yy <- t(means[,var_indices])
			curve <- 0
		} else {
			yy <- mplus.get.data(file,mean_vars,start:(start+count-1))
		}

	}

	# get the time scores
	xx <- array(0,c(dims[1],count))
	for (i in c(1:count)) {
		xx[,i] <- proc$time_scores[,1]
	}
	symb <- array(c(21,22,23,24,25),c(dims[1]))
	colors <- rainbow(dims[2])
	...
	if (curve == 0) {
		...
		if (showmean) {
			if (showadjusted) {
				if (is.null(classidx) || classidx == 0) {
					if (estimated) {
						cstr <- paste("Adjusted estimated means and estimated individual values for",procstr,",",colnames(dfcovariates)[namecol+1])
					} else {
						cstr <- paste("Adjusted estimated means and observed individual values for",procstr,",",colnames(dfcovariates)[namecol+1])
					}
				} else {
					if (estimated) {
						cstr <- paste("Adjusted estimated means and estimated individual values for ",procstr,",",colnames(dfcovariates)[namecol+1],", Class ",classidx,sep="")
					} else {
						cstr <- paste("Adjusted estimated means and observed individual values for ",procstr,",",colnames(dfcovariates)[namecol+1],", Class ",classidx,sep="")
					}
				}
			} else {
				if (is.null(classidx) || classidx == 0) {
					if (estimated) {
						cstr <- paste("Estimated means and estimated individual values for",procstr)
					} else {
						cstr <- paste("Estimated means and observed individual values for",procstr)
					}
				} else {
					if (estimated) {
						cstr <- paste("Estimated means and estimated individual values for ",procstr,", Class ",classidx,sep="")
					} else {
						cstr <- paste("Estimated means and observed individual values for ",procstr,", Class ",classidx,sep="")
					}
				}
			}
		} else {
			if (estimated) {
				cstr <- paste("Estimated individual values for",procstr)
			} else {
				cstr <- paste("Observed individual values for",procstr)
			}
		}
		plot(xx,yy,xlab="",ylab="",main=cstr,type='n',pch=19,lty=2)

		#df <- data.frame()
		if (count == 1) {
			#df <- rbind(df, xx[,1], yy)
			lines(xx[,1],yy,type='b',lty=2)
		} else {
			for (i in c(1:count)) {
				#df <- rbind(df, xx[,i], yy[,i])
				lines(xx[,i],yy[,i],type='b',lty=2)
			}
		}

		if (showmean) {
			if (showadjusted) {
				means <- mplus.get.adjusted.estimated.means(file, procstr, dfcovariates=dfcovariates)
			} else {
				means <- mplus.get.estimated_means(file, procstr)
			}
			if (is.null(classidx) || classidx == 0) {
				for (i in c(1:dims[2])) {
					# means <- mplus.get.estimated_means(file, procstr, i)
					lines(xx[,i],means[,i],type='b',pch=symb[i],lty=1,lwd=2,col=colors[i])
				}
			} else {
				# means <- mplus.get.estimated_means(file, procstr, classidx)
				lines(xx[,classidx],means[,classidx],type='b',pch=symb[classidx],lty=1,lwd=2,col=colors[classidx])
			}
		}
	} else {
		if (showmean) {
			if (showadjusted) {
				if (is.null(classidx) || classidx == 0) {
					cstr <- paste("Adjusted estimated means and observed fitted individual values for",procstr,",",colnames(dfcovariates)[namecol+1])
				} else {
					cstr <- paste("Adjusted estimated means and observed fitted individual values for ",procstr,",",colnames(dfcovariates)[namecol+1],", Class ",classidx,sep="")
				}
			} else {
				if (is.null(classidx) || classidx == 0) {
					cstr <- paste("Estimated means and observed fitted individual values for",procstr)
				} else {
					cstr <- paste("Estimated means and observed fitted individual values for ",procstr,", Class ",classidx,sep="")
				}
			}
		} else {
			cstr <- paste("Observed fitted individual values for",procstr)
		}
		plot(xx,yy,xlab="",ylab="",main=cstr,type='n',pch=19,lty=2)

		if (curve < 0 || curve > 2) {
			stop("- only linear and quadratic curves are supported; use 1 or 2 for the curve argument: ", curve, "\n")
		}
		if (count == 1) {
			#df <- data.frame(x=xx[,1], y=yy)
			if (curve == 1) {
				fit <- lm(yy~xx, data=df)
			} else {
				fit <- lm(yy~poly(xx,2,raw=TRUE), data=df)
			}
			lines(xx, predict(fit, data.frame(x=xx)), type='b',lty=2)
		} else {
			for (i in c(1:count)) {
				#df <- data.frame(x=xx[,1], y=yy[,i])
				if (curve == 1) {
					fit <- lm(yy[,i]~xx[,i], data=df)
				} else {
					fit <- lm(yy[,i]~poly(xx[,1],2,raw=TRUE), data=df)
				}
				lines(xx[,i], predict(fit, data.frame(x=xx[,i])), type='b',lty=2)
			}
		}
...
##########################################################################
#
# mplus.compute.individual.estimated.values - compute the estimated means based on given data frame of covariate values
#
# arguments:
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.compute.individual.estimated.values('ex8.1.gh5',dfx,)
#
mplus.compute.individual.estimated.values <- function(file,obs=NULL,class=NULL) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if ( !("individual_data" %in% names(gh5)) ) {
		stop("Individual data is required.\n\nUse TYPE=PLOT3.")
	}
	...
	num_x <- as.integer(props[3])
	num_f <- num_fx - num_x
	num_taus <- as.integer(props[4])
	num_y <- as.integer(props[1]) - as.integer(props[12])
	if (num_taus == 0) {
		num_y <- num_y - num_x
	}

	if (mplus.check.group.attribute(file, 'individual_data', 'grouping')) {
		var_names <- mplus.get.group.dataset(file, 'individual_data', 'var_names')
		var_names <- gsub("(^\\s+|\\s+$)", "", var_names, perl=TRUE)
		var_names <- gsub(":1", ", mean", var_names, perl=FALSE)
		var_names <- gsub(":2", ", median", var_names, perl=FALSE)
		...
		group <- mplus.get.data(file, var_names[length(var_names)], obs)
	} else if (mplus.check.group.attribute(file, 'individual_data', 'classes')) {
		n_classvars <- mplus.get.group.attribute(file, 'individual_data', 'classes')
		if (n_classvars == 0) {
			if (!is.null(class)) {
				group <- mplus.get.data(file, "CLASS", class=class)
			} else if (!is.null(obs)) {
				group <- mplus.get.data(file, "CLASS", obs)
			} else {
				group <- mplus.get.data(file, "CLASS")
			}
		}
	} else {
		group <- 1
	...
	n_obs_fscores <- 0
	if (mplus.check.group.attribute(file, 'individual_data', 'n_obs_fscores')) {
		n_obs_fscores <- as.integer(mplus.get.group.attribute(file, 'individual_data', 'n_obs_fscores'))
	}

	if (is.null(gh5$matrix_data$betam1) && is.null(gh5$matrix_data$betam2) && n_obs_fscores == 0) {
		# use mu2 and betaq2
		if (num_x > 0) {
			if (!is.null(class)) {
				xvalues <- mplus.get.data(file,class=class,vtype=1)
			} else if (!is.null(obs)) {
				xvalues <- mplus.get.data(file,obs=obs,vtype=1)
			} else {
				xvalues <- mplus.get.data(file,vtype=1)
			}
		}

		if (!is.null(class) || is.null(obs) || (is.vector(obs) && length(obs) > 1)) {
			means <- array(NA, c(num_y,length(group)))
			for (i in 1:length(group)) {
				g <- group[i]
				x <- xvalues[,i]
				means[,i] <- gh5$matrix_data$mu2[1:num_y,g]

				if (num_x > 0) {
					means[,i] <- means[,i] + gh5$matrix_data$betaq2[,,g] %*% x
				}
			}
		} else {
			means <- gh5$matrix_data$mu2[1:num_y,group]

			if (num_x > 0) {
				means <- means + gh5$matrix_data$betaq2[,,group] %*% xvalues
			}
		}
	} else {
		# use mu1, betaq1 and betam1
		means <- gh5$matrix_data$mu1[1:num_y,group]

		if (num_x > 0) {
			if (!is.null(class)) {
				x1 <- mplus.get.data(file,class=class,vtype=1)
			} else if (!is.null(obs)) {
				x1 <- mplus.get.data(file,obs=obs,vtype=1)
			} else {
				x1 <- mplus.get.data(file,vtype=1)
			}
			if (num_x == 1) {
				xvalues <- array(0,c(num_x,length(x1)))
				xvalues[1,] <- x1
			} else {
				xvalues <- x1
			}
			means <- means + gh5$matrix_data$betaq1[1:num_y,,group] %*% xvalues
		}

		if (mplus.check.group.attribute(file, 'individual_data', 'n_fscores')) {
			n_fscores <- mplus.get.group.attribute(file, 'individual_data', 'n_fscores')
			if (!is.null(class)) {
				fscores1 <- mplus.get.data(file,class=class,vtype=2)
			} else if (!is.null(obs)) {
				fscores1 <- mplus.get.data(file,obs=obs,vtype=2)
			} else {
				fscores1 <- mplus.get.data(file,vtype=2)
			}
			if (n_fscores == 1) {
				fscores <- array(0,c(n_fscores,length(fscores1)))
				fscores[1,] <- fscores1
			} else {
				fscores <- fscores1
			}
			means <- means + gh5$matrix_data$betam1[1:num_y,,group] %*% fscores
		}
	}
	invisible(t(means))
}
...
##########################################################################
#
# mplus.compute.individual.estimated.probabilities - compute the estimated means based on given data frame of covariate values
#
# arguments:
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.compute.individual.estimated.probabilities('ex8.1.gh5',dfx,)
#
mplus.compute.individual.estimated.probabilities <- function(file,obs=NULL,class=NULL) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (is.null(gh5$matrix_data$betam1) && is.null(gh5$matrix_data$betam2) && n_obs_fscores == 0) {
		...
		# use m1, betaq1 and betam1
		means <- gh5$matrix_data$mu1[1:num_y,group]

		if (mplus.check.group.attribute(file, 'individual_data', 'n_fscores')) {
			fscores <- mplus.get.data(file,obs=obs,vtype=2)
			means <- means + gh5$matrix_data$betam1[,,group] %*% fscores
		}
	}
	invisible(t(means))
}
...
#
mplus.list.variables <- function(file, quiet=FALSE, eonly=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (("matrix_data" %in% names(gh5)))
	{
		if (mplus.check.group.attribute(file, 'individual_data', 'n_y'))
		{
			uoffset <- 0
			n_y <- as.integer(mplus.get.group.attribute(file, 'individual_data', 'n_y'))

			if (mplus.check.group.attribute(file, 'individual_data', 'u_offset'))
			{
				uoffset <- as.integer(mplus.get.group.attribute(file, 'individual_data', 'u_offset'))
				n_y <- n_y - uoffset
			}

			if (mplus.check.group.attribute(file, 'individual_data', 'n_x'))
			{
				n_x <- as.integer(mplus.get.group.attribute(file, 'individual_data', 'n_x'))
				n_y <- n_y - n_x
			}
			...
			if (n_y > 0)
			{
				if (!quiet)
				{
					cat("\nEstimated values and residuals are available for the following variables:\n")
				}
				len <- length(var_names)
				for (i in c(1:n_y))
				{
					evars <- append(evars, var_names[uoffset+i])
					if (!quiet)
					{
						cstr <- sprintf("[%d] %s\n", i, var_names[uoffset+i])
						cat(cstr)
					}
				}
			}
...
#
mplus.get.data <- function(file,v,obs=NULL,class=NULL,vtype=NULL) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (!missing(v)) {
		if (is.array(v) || is.vector(v)) {
			index <- vector()
			for (i in c(1:length(v))) {
				if (is.numeric(v[i])) {
					if (v[i] > length(var_names) || v[i] < 1) {
					...
					vindex <- pmatch(var, var_names, nomatch=0)
					if (vindex == 0) {
						cstr <- paste(c("Unknown variable:"),var,"\n")
						stop(cstr)
					}
					index <- append(index, vindex)
				}
			}
		} else {
			if (is.numeric(v)) {
				if (v > length(var_names) || v < 1) {
					stop("- variable index out of bounds.\n\nUse mplus.list.variables() to get the list of variable names.")
				}
		...
		# order of variables: n_y (including x), n_training, n_aux, n_fscores
		if (vtype == 1) {
			# x variables
			num_y <- as.integer(mplus.get.group.attribute(file, 'individual_data', 'n_y'))
			num_x <- as.integer(mplus.get.group.attribute(file, 'individual_data', 'n_x'))
			x_start <- num_y - num_x + 1
			index <- seq(x_start,num_y)
		} else if (vtype == 2) {
			# fscores
			if (mplus.check.group.attribute(file, 'individual_data', 'n_fscores')) {
				num_fscores <- as.integer(mplus.get.group.attribute(file, 'individual_data', 'n_fscores'))
				start <- as.integer(mplus.get.group.attribute(file, 'individual_data', 'n_y'))
				if (mplus.check.group.attribute(file, 'individual_data', 'n_training')) {
					start <-start + as.integer(mplus.get.group.attribute(file, 'individual_data', 'n_training'))
				}
				if (mplus.check.group.attribute(file, 'individual_data', 'n_aux')) {
					start <- start + as.integer(mplus.get.group.attribute(file, 'individual_data', 'n_aux'))
				}

				start <- start + 1
				end <- start + num_fscores - 1
				index <- seq(start, end)
			} else {
				stop("- no factor scores.\n\nUse mplus.list.variables() to get the list of variable names.")
			}
		}
	} else {
	...
	if (!is.null(class)) {
		if (!mplus.check.group.attribute(file,'individual_data','classes')) {
			stop(" - individual_data has no classes; remove the 'class' argument")
		}
		classvar <- pmatch("CLASS", var_names, nomatch=0)
		classes <- gh5$individual_data$raw_data[classvar,]
		xx <- gh5$individual_data$raw_data[index,which(classes == class)]
	} else if (!is.null(obs)) {
		dims <- attr(gh5$individual_data$raw_data, 'dim')
		if (is.vector(obs)) {
			for (i in c(1:length(obs))) {
				if (obs[i] < 1 || obs[i] > dims[2]) {
					cstr <- paste(" - index for observation out of range; range is between 1 and ", dims[2],":  ", obs[i], "\n")
					stop(cstr)
				}
			}
		} else {
			if (obs < 1 || obs > dims[2]) {
				cstr <- paste(" - index for observation out of range; range is between 1 and ", dims[2],":  ", obs, "\n")
				stop(cstr)
			}
		}

		# get the data for the variable
		xx <- gh5$individual_data$raw_data[index,obs]
	} else {
		# get the data for the variable
...
##########################################################################
#
# mplus.get.between.data - return the between level data for the quoted variable
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	v - name of variable to plot
#	clusvar - the cluster ID for the desired level
#
# eg. mplus.get.between.data('ex8.1.gh5','y1')
#
mplus.get.between.data <- function(file,v,clusvar=NULL) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (missing(clusvar)) {
		...
		if (nclusters == 1) {
			...
			clusindex <- length(var_names)
		} else {
			clusvar <- var_names[length(var_names)-1]
			clusindex <- length(var_names)-1
		}
	} else {
		clusindex <- pmatch(clusvar, var_names, nomatch=0)
		if (clusindex == 0) {
	...
	ids <- gh5$individual_data$raw_data[clusindex,]
	vids <- ids
	ids <- sort(unique(ids))

	# get the variable names from individual_data and lookup the indices
	var_levels <- mplus.get.group.attribute(file, 'individual_data', 'levels')
	...
	if (is.array(v) || is.vector(v)) {
		...
		for (i in c(1:length(v))) {
			if (is.numeric(v[i])) {
				...
				if (var_levels[v[i]] < 2) {
					cstr <- paste("- variable", var_names[v[i]], "is not a between variable.  Between variables are level 2 or above.\n\n")
					stop(cstr)
				}
				...
				if (var_levels[vindex] < 2) {
					cstr <- paste("- variable", var, "is not a between variable.  Between variables are level 2 or above.\n\n")
					stop(cstr)
				}
		...
		if (is.numeric(v)) {
			...
			if (var_levels[v] < 2) {
				cstr <- paste("- variable", var_names[v], "is not a between variable.  Between variables are level 2 or above.\n\n")
				stop(cstr)
			}
	...
	vdata <- array(NA,c(length(index),length(ids)))
	for (i in c(1:length(ids))) {
		vdata[,i] <- gh5$individual_data$raw_data[index,which(vids == ids[i])][1]
	}
...
mplus.list.cluster.idnums <- function(file,clusvar,fprint=FALSE) {
	...
	ids <- mplus.get.data(file, index)
	ids <- sort(unique(ids))

#	cat(c("\nList of variable names to use in the following functions:\n"))
#	cat(c(" - mplus.plot.timeseries.observed\n"))
...
#
mplus.plot.scatterplot <- function(file, xv, yv, between=FALSE, getdf=FALSE, xestimated=FALSE, yestimated=FALSE, xresidual=FALSE, yresidual=FALSE, group=0) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (between) {
		varlist <- mplusapp.get.variable.list.between.plots(file)
		if (is.numeric(xv)) {
			if (xv < 1 || xv > length(varlist)) {
				cstr <- paste("- between variable index is out of range, should be between 1 and ",length(varlist),":\n")
				stop(cstr)
			}
			xvar <- varlist[xv]
		}
		if (is.numeric(yv)) {
			if (yv < 1 || yv > length(varlist)) {
				cstr <- paste("- between variable index is out of range, should be between 1 and ",length(varlist),":\n")
				stop(cstr)
			}
			yvar <- varlist[yv]
		}
		betweendf <- mplusapp.find.between.plots.variable(file, c(xvar,yvar))

		if (betweendf['is_clusterid',1]) {
			xx <- mplus.get.data(file,betweendf['variable_name',1])
			xx <- sort(unique(xx))
		} else if (betweendf['is_data',1]) {
			xx <- mplus.get.between.data(file,betweendf['variable_name',1])
		} else if (betweendf['sample_over_within',1]) {
			if (betweendf['mean_value',1]) {
				xx <- mplus.get.values.over.within.cluster(file,var=betweendf['variable_name',1])
			} else {
				xx <- mplus.get.values.over.within.cluster(file,var=betweendf['variable_name',1],vtype=2)
			}
		} else if (betweendf['estimated_cluster_value',1]) {
			if (betweendf['mean_value',1]) {
				xx <- mplus.get.estimated.cluster.values(file,var=betweendf['variable_name',1])
			} else {
				xx <- mplus.get.estimated.cluster.values(file,var=betweendf['variable_name',1],vtype=2)
			}
		} else {
			cstr <- paste("Unknown between entity:",xv)
			stop(cstr)
		}
		if (betweendf['is_clusterid',2]) {
			yy <- mplus.get.data(file,betweendf['variable_name',2])
			yy <- sort(unique(yy))
		} else if (betweendf['is_data',2]) {
			yy <- mplus.get.between.data(file,betweendf['variable_name',2])
		} else if (betweendf['sample_over_within',2]) {
			if (betweendf['mean_value',2]) {
				yy <- mplus.get.values.over.within.cluster(file,var=betweendf['variable_name',2])
			} else {
				yy <- mplus.get.values.over.within.cluster(file,var=betweendf['variable_name',2],vtype=2)
			}
		} else if (betweendf['estimated_cluster_value',2]) {
			if (betweendf['mean_value',2]) {
				yy <- mplus.get.estimated.cluster.values(file,var=betweendf['variable_name',2])
			} else {
				yy <- mplus.get.estimated.cluster.values(file,var=betweendf['variable_name',2],vtype=2)
			}
		} else {
			cstr <- paste("Unknown between entity:",yv)
			stop(cstr)
		}
		xlabel <- xvar
		ylabel <- yvar
		...
		if (is.numeric(xv)) {
			if (xv <= 0 || xv > length(var_names)) {
				cstr <- paste("- variable index is out of range, should be between 1 and ",length(var_names),":",xv,"\n")
				stop(cstr)
			}
			xindex <- xv
			xvar <- var_names[xv]
		...
		if (is.numeric(yv)) {
			if (yv <= 0 || yv > length(var_names)) {
				cstr <- paste("- variable index is out of range, should be between 1 and ",length(var_names),":",yv,"\n")
				stop(cstr)
			}
			yindex <- yv
			yvar <- var_names[yv]
		...
		if (xestimated || yestimated || xresidual || yresidual) {
			evars <- mplus.list.variables(file,TRUE,TRUE)
			if (xestimated || xresidual)
			{
				eindex <- pmatch(xvar, evars, nomatch=0)
				if (eindex == 0)
				{
					cstr <- paste(c("X-variable does not have estimated values or residuals:"),xvar,"\n")
					stop(cstr)
				}
			}
			if (yestimated || yresidual)
			{
				eindex <- pmatch(yvar, evars, nomatch=0)
				if (eindex == 0)
				{
					cstr <- paste(c("Y-variable does not have estimated values or residuals:"),yvar,"\n")
					stop(cstr)
				}
			}
			if (group > 0) {
				allestimated <- mplus.compute.individual.estimated.values(file,class=group)
			} else {
				allestimated <- mplus.compute.individual.estimated.values(file)
			}
		}
		if (xestimated || xresidual) {
			xx <- allestimated[,xindex]
			if (xresidual) {
				if (group > 0) {
					observed <- mplus.get.data(file,xvar,class=group)
				} else {
					observed <- mplus.get.data(file,xvar)
				}
				xx <- observed - xx
			}
		} else {
			if (group > 0) {
				xx <- mplus.get.data(file,xvar,class=group)
			} else {
				xx <- mplus.get.data(file,xvar)
			}
		}
		if (yestimated || yresidual) {
			yy <- allestimated[,yindex]
			if (yresidual) {
				if (group > 0) {
					observed <- mplus.get.data(file,yvar,class=group)
				} else {
					observed <- mplus.get.data(file,yvar)
				}
				yy <- observed - yy
			}
		} else {
			if (group > 0) {
				yy <- mplus.get.data(file,yvar,class=group)
			} else {
				yy <- mplus.get.data(file,yvar)
			}
		}
		if (xestimated) {
			xlabel <- paste(xvar,c("(estimated)"))
		} else if (xresidual) {
			xlabel <- paste(xvar,c("(residual)"))
		} else {
			xlabel <- xvar
		}
		if (yestimated) {
			ylabel <- paste(yvar,c("(estimated)"))
		} else if (yresidual) {
			ylabel <- paste(yvar,c("(residual)"))
		} else {
			ylabel <- yvar
		}
	}

	plot(xx,yy,xlab=xlabel,ylab=ylabel)
...
#
mplus.plot.histogram <- function(file,v=1,bins=20,between=FALSE,getdf=FALSE,estimated=FALSE,residual=FALSE,group=0) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (between) {
		if (is.numeric(v)) {
			varlist <- mplusapp.get.variable.list.between.plots(file)
			if (v < 1 || v > length(varlist)) {
				cstr <- paste("- between variable index is out of range, should be between 1 and ",length(varlist),":\n")
				stop(cstr)
			}
			v <- varlist[v]
		}
		betweendf <- mplusapp.find.between.plots.variable(file, v)

		if (betweendf['is_clusterid',1]) {
			xx <- mplus.get.data(file,betweendf['variable_name',1])
			xx <- sort(unique(xx))
		} else if (betweendf['is_data',1]) {
			if (mplus.has.significance(file,betweendf['variable_name',1])) {
				sig <- mplus.get.significance(file,betweendf['variable_name',1],between=TRUE)
				dd <- mplus.get.between.data(file,betweendf['variable_name',1])
			...
			cstr <- paste("Unknown between entity:",v)
			stop(cstr)
		}
		cstr <- paste(c("Between-level histogram of"),v)
		var <- betweendf['variable_name',1]
		xlabel <- var
	} else {
		# get the variable names from individual_data and lookup the indices
		var_names <- mplus.get.group.dataset(file, 'individual_data', 'var_names')
		...
		if (is.character(v)) {
			...
			if (v <= 0 || v > length(var_names)) {
				cstr <- paste("- variable index is out of range, should be between 1 and ",length(var_names),":\n")
				stop(cstr)
			}
			index <- v
			var <- var_names[v]
		...
		if (estimated || residual) {
			evars <- mplus.list.variables(file,TRUE,TRUE)
			eindex <- pmatch(var, evars, nomatch=0)
			if (eindex == 0)
			{
				cstr <- paste(c("Variable does not have estimated values or residuals:"),var,"\n")
				stop(cstr)
			}
			if (group > 0) {
				allestimated <- mplus.compute.individual.estimated.values(file,class=group)
			...
			xx <- allestimated[,index]
			if (residual) {
				if (group > 0) {
					observed <- mplus.get.data(file,v,class=group)
				} else {
					observed <- mplus.get.data(file,v)
				}
				xx <- observed - xx
			}
		} else {
			if (group > 0) {
				xx <- mplus.get.data(file,v,class=group)
			} else {
				xx <- mplus.get.data(file,v)
			}
		}
		if (estimated) {
			xlabel <- paste(var,c("(estimated)"))
		} else if (residual) {
			xlabel <- paste(var,c("(residual)"))
		} else {
			xlabel <- var
		}
		cstr <- paste(c("Histogram of"),xlabel)
	}
	if (has_significance) {
		h1 <- hist(xx,breaks=seq(min(dd, na.rm=TRUE),max(dd, na.rm=TRUE),length=bins+1))
		h2 <- hist(x2,breaks=seq(min(dd, na.rm=TRUE),max(dd, na.rm=TRUE),length=bins+1))$counts
		h3 <- hist(x3,breaks=seq(min(dd, na.rm=TRUE),max(dd, na.rm=TRUE),length=bins+1))$counts
		xlabels <- sprintf("%0.4f", h1$mid)
		barplot(rbind(h1$counts,h2,h3),space=0.1,las=2,main=cstr,xlab=xlabel,col=c('yellow','gray','darkgreen'),names.arg=xlabels,cex.names=0.7)

		total <- sum(h1$counts)+sum(h2)+sum(h3)
		legtxt1 <- sprintf("%2.1f%% significant negative", sum(h1$counts)/total*100)
		legtxt2 <- sprintf("%2.1f%% not significant", sum(h2)/total*100)
		legtxt3 <- sprintf("%2.1f%% significant positive", sum(h3)/total*100)
		legend("topright",                                    # Add legend to barplot
				legend = c(legtxt1,legtxt2,legtxt3),
				fill = c("yellow", "gray", "darkgreen"))

		if (getdf) {
			df <- data.frame(h1$mids, h1$counts, h2, h3)
			colnames(df) <- c('Mids','Significant negative','Not significant','Significant positive')
			df
		}
	} else {
		h <- hist(xx,breaks=seq(min(xx, na.rm=TRUE),max(xx, na.rm=TRUE),length=bins+1),col="red",main=cstr,xlab=xlabel,right=TRUE)
...
#
mplus.plot.densityplot <- function(file,v=1,bins=20,between=FALSE,estimated=FALSE,residual=FALSE,group=0,skipsig=FALSE) {
	if (missing(file)) {
		stop("name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (between) {
		...
		if (betweendf['is_clusterid',1]) {
			...
			if (mplus.has.significance(file,betweendf['variable_name',1]) && !skipsig) {
				sig <- mplus.get.significance(file,betweendf['variable_name',1],between=TRUE)
				dd <- mplus.get.between.data(file,betweendf['variable_name',1])
				values <- dd[1,]
				significance <- sig[1,]
				significance[significance == -1] <- "Negative"
				significance[significance == 0] <- "Not"
				significance[significance == 1] <- "Positive"

				dadf <- data.frame(values)
				ddf <- data.frame(values,significance)
				ddf$significance <- factor(ddf$significance, levels = c("Positive", "Not", "Negative"))
		...
		cstr <- paste(c("Between-level density plot of"),v)
		var <- betweendf['variable_name',1]
		xlabel <- var
	} else {
		# get the variable names from individual_data and lookup the indices
		...
		cstr <- paste(c("Density plot of"),xlabel)
	}
	...
	if (has_significance) {
		...
		mn <- mean(dd,na.rm=TRUE)
		dfmean <- data.frame(mn)

		ggplot(ddf, aes(x=values,fill=significance,color=significance)) + geom_histogram(breaks=h1$breaks)+xlab(v)+ggtitle(cstr)+
		theme(plot.title = element_text(hjust = 0.5))+
		scale_fill_manual(values=c("darkgreen", "gray", "yellow"),name=NULL,labels=c(legtxt3, legtxt2, legtxt1))+
		scale_color_manual(values=c("black", "black", "black","red"),name=NULL,labels=c(legtxt3, legtxt2, legtxt1),guide='none')+
		geom_vline(data=dfmean, aes(xintercept=mn,color='red'),linewidth=1,show.legend=FALSE)
...
#		}
		...
		hist(xx,freq=FALSE,breaks=seq(min(xx,na.rm=TRUE),max(xx,na.rm=TRUE),length=bins+1),col="red",main=cstr,xlab=xlabel,right=TRUE)
		mn <- mean(xx,na.rm=TRUE)
		std <- sd(xx,na.rm=TRUE)
...
##########################################################################
#
# mplus.plot.qqplot - plot the QQ plot for the quoted variable
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	v - name of variable to plot
...
#
# eg. mplus.plot.qqplot('y1',5)
#
mplus.plot.qqplot <- function(file,v=1,datax=FALSE,line=TRUE,between=FALSE,estimated=FALSE,residual=FALSE,group=0,main=NULL,xlab="Theoretical Quantiles",ylab=NULL) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	# check if individual data exists
	if ( !("individual_data" %in% names(gh5)) ) {
		stop("mplus.plot.qqplot requires individual data.\n\nUse TYPE=PLOT1 or TYPE=PLOT3 setting in Mplus to store individual data\nfor histograms.")
	}
	...
	if (between) {
		...
		cstr <- paste(c("Between-level QQ plot of"),v)
		var <- betweendf['variable_name',1]
		xlabel <- var
	} else {
		# get the variable names from individual_data and lookup the indices
		...
		cstr <- paste(c("QQ plot of"),xlabel)
	}
	...
	yy <- qqnorm(xx,plot.it=FALSE)
	zz <- mean(xx, na.rm=TRUE) + sd(xx, na.rm=TRUE) * yy$x
	qqplot(zz, xx, main=main, xlab=xlab, ylab=ylab, col='red')
...
#
mplus.list.bayesian.parameters <- function(file,quiet=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
#
mplus.list.bayesian.predictive.labels <- function(file, quiet=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
#
mplus.list.loop.labels <- function(file, quiet=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (!quiet) {
		for (i in c(1:length(statements))) {
			cstr <- sprintf("[%d] %s", i, statements[i])
			cat(cstr,sep="\n")
		}
	#	cat(statements,sep="\n")
...
mplus.plot.loop <- function(file,label=1,showgrid=TRUE,ylim,linecolors,linetype,noci=FALSE,xlabel,ylabel) {
	...
	if (length(label) > 1) {
		...
		for (r in c(1:num_loop)) {
		  if (noci) {
		    lines(xx[(r-1)+1,],yy[(r-1)+1,],col=linecolors[r]) #, pch=plotchar[r])
		  } else {
...
#
mplus.list.moderation.labels <- function(file, quiet=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
#
mplus.list.sensitivity.labels <- function(file, quiet=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
#
mplus.plot.irt.icc <- function(file,group=1,xvar=1,uvar,cat,cat2,covariates,xrange=3,xstep=7,lloc="top",getdf=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (!is.numeric(group)) {
		grouplist <- mplus.get.model_group_labels(file)
		group <- pmatch(group, grouplist,nomatch=0)
	}
	if (group <= 0 || group > props[4]) {
		stop("The group index (group) is out of range.")
	}
	...
	if (missing(uvar)) {
		...
		rnames <- array(0,categories[ridx])
		...
		prob <- array(0,c(1,length(steps)))
		rnames <- array(0,2)

		prob[1,] <- mplus.compute.irt.icc(file,group,fidx,ridx,cat,steps,covariates)

		# plot the icc
		ldesc <- sprintf("%s, Category %d, Class %d", ulabels[ridx], cat, group)
		...
		for (c in c(cat:cat2)) {
			prob[1,] <- prob[1,] + mplus.compute.irt.icc(file,group,fidx,ridx,c,steps,covariates)
		}

		# plot the icc
		ldesc <- sprintf("%s, Sum from category %d to category %d, Class %d", ulabels[ridx], cat, cat2, group)
	...
	if (getdf) {
		...
		df <- rbind(steps, df)
		rownames(df) <- rnames
		colnames(df) <- seq(1,length(steps))
		df <- t(df)
		df
	}
}
...
#
mplus.list.survival.variables <- function(file,quiet=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	bdiscrete <- FALSE
	surv_group <- c('survival_data')
	if ( !(surv_group %in% names(gh5)) ) {
		surv_group <- c('discrete_survival_data')
		if ( !(surv_group %in% names(gh5)) ) {
	...
	props <- mplus.get.group.attribute(file,surv_group,'properties')
...
#	cat(c(" - mplus.plot.irt.icc\n"))
	...
	for (i in c(1:props[1])) {
		cstr <- sprintf("%s/survival%d", surv_group, i)
		label <- mplus.get.group.attribute(file,cstr,'label')
		varlist <- append(varlist, label)
		if (!quiet) {
...
#
mplus.get.survival.kaplanmeier.values <- function(file,survvar,classnum) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	props <- mplus.get.group.attribute(file,surv_group,'properties')
	survlist <- mplus.list.survival.variables(file,TRUE)

	if (is.character(survvar)) {
		surv_idx <- pmatch(survvar, survlist, nomatch=0)
		if (surv_idx == 0) {
	...
	surv_str <- sprintf("survival%d", surv_idx)
	if (missing(classnum)) {
	...
	if (bdiscrete) {
		kmvals <- gh5$discrete_survival_data[[surv_str]][[datastr]]
	} else {
		kmvals <- gh5$survival_data[[surv_str]][[datastr]]
	}

	df <- data.frame()
	values <- kmvals[,2]
	time <- kmvals[,1]
	df <- t(rbind(df, values, time))
	rownames(df) <- seq(1,length(time))
	colnames(df) <- c('Values', 'Time')
	df
}
...
#
mplus.compute.survival.sample.logcumulative.values <- function(file,survvar,classnum) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
#	file - the quoted name of an existing GH5 file
#	survvar - the quoted name of the survival variable or the index of the survival variable (required)
#	endvar - ending survival variable for getting sequential time
#	classnum - the group number (not required)
#
# eg. mplus.get.survival.baseline.values('ex6.21.gh5','T')
#
mplus.get.survival.baseline.values <- function(file,survvar,endvar,classnum) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (!(missing(endvar))) {
		if (is.character(endvar)) {
			end_idx <- pmatch(endvar, survlist, nomatch=0)
			if (end_idx == 0) {
				stop("- unknown survival variable:  ", endvar)
			}
		} else {
			if (endvar <= 0 || endvar > props[1]) {
				stop("- index for the survival variable is out of range")
			}
			end_idx <- endvar
		}
	...
	if (missing(endvar)) {
		...
		if (bdiscrete) {
			esvals <- gh5$discrete_survival_data[[surv_str]][[datastr]]
		} else {
			esvals <- gh5$survival_data[[surv_str]][[datastr]]
		}
		...
		values <- vector()
		time <- vector()
		for (s in c(surv_idx:end_idx)) {
			surv_str <- sprintf("survival%d", s)
			if (missing(classnum)) {
				datastr <- sprintf("estimated_survival1")
			} else {
				datastr <- sprintf("estimated_survival%d", classnum)
			...
			if (bdiscrete) {
				esvals1 <- gh5$discrete_survival_data[[surv_str]][[datastr]]
			} else {
				esvals1 <- gh5$survival_data[[surv_str]][[datastr]]
			}
			edim <- attr(esvals1,"dim")

			if (s == surv_idx) {
				values <- append(values, esvals1[,2])
				time <- append(time, esvals1[,1])
			} else {
				values <- append(values, data_last * esvals1[2:edim[1],2])
				time <- append(time, time_last + esvals1[2:edim[1],1])
			}
			data_last <- values[length(values)]
			time_last <- time[length(time)]
		}
		df <- t(rbind(df, values, time))
	}
	rownames(df) <- seq(1,length(time))
...
#
mplus.compute.survival.estimated.logcumulative.values <- function(file,survvar,endvar,classnum) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (missing(endvar)) {
		...
		for (s in c(surv_idx:end_idx)) {
			...
			if (s == surv_idx) {
				values <- append(values, log(-log(esvals1[,2])))
				time <- append(time, esvals1[,1])
				data_last <- esvals1[edim[1],2]
			} else {
				values <- append(values, log(-log(data_last * esvals1[2:edim[1],2])))
				time <- append(time, time_last + esvals1[2:edim[1],1])
				data_last <- data_last * esvals1[edim[1],2]
			}
			time_last <- time[length(time)]
		}
		df <- t(rbind(df, values, time))
...
#
mplus.get.survival.basehazard.values <- function(file,survvar,classnum) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	surv_str <- sprintf("survival%d", surv_idx)
	if (missing(classnum)) {
		datastr <- sprintf("basehazard1")
	} else {
		datastr <- sprintf("basehazard%d", classnum)
	}
	bhvals <- gh5$survival_data[[surv_str]][[datastr]]
...
mplus.plot.survival.kaplanmeier <- function(file,survvar=1,classnum) {
	...
	label <- survlist[surv_idx]

	classes <- mplus.get.group.dataset(file,'/','model_group_labels')
	classes <- gsub("(^\\s+|\\s+$)", "", classes, perl=TRUE)
	dims <- attr(classes,'dim')
	...
	if (missing(classnum)) {
		...
		for (i in c(1:dims[1])) {
			dfi <- mplus.get.survival.kaplanmeier.values(file,surv_idx,i)
			npoints[i] <- length(dfi[,1])
		}
		...
		df <- mplus.get.survival.kaplanmeier.values(file,surv_idx,classnum)

		plot(df[,2],df[,1],xlab="Time",ylab="Probability",main=cstr,type='n',ylim=c(0,1))
		lines(df[,2],df[,1],col='red')
	}
}
...
#
mplus.plot.survival.baseline <- function(file,survvar=1,classnum,endvar=NULL) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (!is.null(endvar)) {
		...
		elabel <- survlist[end_idx]
		cstr <- sprintf("Estimated baseline survival curve for %s to %s", label, elabel)
	} else {
	...
	if (missing(classnum)) {
		...
		for (i in c(1:dims[1])) {
			if (is.null(endvar)) {
				dfi <- mplus.get.survival.baseline.values(file,surv_idx,classnum=i)
			} else {
				dfi <- mplus.get.survival.baseline.values(file,surv_idx,end_idx,classnum=i)
			}
		...
		if (is.null(endvar)) {
			df <- mplus.get.survival.baseline.values(file,surv_idx,classnum=classnum)
		} else {
			df <- mplus.get.survival.baseline.values(file,surv_idx,end_idx,classnum)
		}
...
mplus.plot.survival.basehazard <- function(file,survvar=1,classnum) {
	...
	if (missing(classnum)) {
		npoints <- array(0, c(dims[1]))
		for (i in c(1:dims[1])) {
			dfi <- mplus.get.survival.basehazard.values(file,surv_idx,i)
			npoints[i] <- length(dfi[,1])
		}
		maxpoints <- max(npoints)
		...
		df <- mplus.get.survival.basehazard.values(file,surv_idx,classnum)

		plot(df[,2],df[,1],xlab="Time",ylab="",main=cstr,type='n')
		lines(df[,2],df[,1],col='red')
	}
}
...
mplus.plot.survival.sample.logcumulative <- function(file,survvar=1,classnum) {
	...
	if (missing(classnum)) {
		...
		for (i in c(1:dims[1])) {
			dfi <- mplus.compute.survival.sample.logcumulative.values(file,surv_idx,i)
			xall[i,] <- dfi[,2]
			yall[i,] <- dfi[,1]
			for (j in c(1:npoints[i])) {
				if (is.infinite(yall[i,j])) {
					xall[i,j] <- NA
				}
			}
		}
		...
		df <- mplus.compute.survival.sample.logcumulative.values(file,surv_idx,classnum)
		for (j in c(1:length(df[,2]))) {
			if (is.infinite(df[j,1])) {
				df[j,2] <- NA
			}
		}
...
#
mplus.plot.survival.estimated.logcumulative <- function(file,survvar=1,classnum,endvar=NULL) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (!is.null(endvar)) {
		...
		elabel <- survlist[end_idx]
		cstr <- sprintf("Estimated log cumulative baseline hazard curve for %s to %s", label, elabel)
	} else {
		cstr <- sprintf("Estimated log cumulative baseline hazard curve for %s", label)
	}
	...
	if (missing(classnum)) {
		...
		for (i in c(1:dims[1])) {
			if (is.null(endvar)) {
				dfi <- mplus.compute.survival.estimated.logcumulative.values(file,surv_idx,classnum=i)
			} else {
				dfi <- mplus.compute.survival.estimated.logcumulative.values(file,surv_idx,end_idx,classnum=i)
			}
			xall[i,] <- dfi[,2]
			yall[i,] <- dfi[,1]
			for (j in c(1:npoints[i])) {
		...
		if (is.null(endvar)) {
			df <- mplus.compute.survival.estimated.logcumulative.values(file,surv_idx,classnum=classnum)
		} else {
			df <- mplus.compute.survival.estimated.logcumulative.values(file,surv_idx,end_idx,classnum)
		}
...
mplus.plot.survival.kaplanmeier.vs.baseline <- function(file,survvar=1,classnum) {
	...
	if (missing(classnum)) {
		...
		for (i in c(1:dims[1])) {
			dfi <- mplus.get.survival.kaplanmeier.values(file,surv_idx,i)
			npoints[2*(i-1)+1] <- length(dfi[,1])
			dfi <- mplus.get.survival.baseline.values(file,surv_idx,classnum=i)
			npoints[2*i] <- length(dfi[,1])
		}
		maxpoints <- max(npoints)
		...
		for (i in c(1:dims[1])) {
			dfi <- mplus.get.survival.kaplanmeier.values(file,surv_idx,i)
			xall[2*(i-1)+1,1:length(dfi[,2])] <- dfi[,2]
			yall[2*(i-1)+1,1:length(dfi[,1])] <- dfi[,1]

			dfi <- mplus.get.survival.baseline.values(file,surv_idx,classnum=i)
			xall[2*i,1:length(dfi[,2])] <- dfi[,2]
			yall[2*i,1:length(dfi[,1])] <- dfi[,1]
		}
		plot(xall,yall,xlab="Time",ylab="Probability",main=cstr,type='n',ylim=c(0,1))
		...
		for (i in c(1:dims[1])) {
			ldesc[2*(i-1)+1] <- sprintf("KM for %s", classes[i])
			lty[2*(i-1)+1] = 1
			lwd[2*(i-1)+1] = 2.5

			ldesc[2*i] <- sprintf("ES for %s", classes[i])
			lty[2*i] = 1
			lwd[2*i] = 2.5
		}
		legend("top",ldesc,col=colors,lty=lty,lwd=lwd)
		...
		npoints <- array(0, c(2))
		km <- mplus.get.survival.kaplanmeier.values(file,surv_idx,classnum)
		npoints[1] <- length(km[,1])
		bv <- mplus.get.survival.baseline.values(file,surv_idx,classnum=classnum)
		npoints[2] <- length(bv[,1])
		maxpoints <- max(npoints)

		xall <- array(NA, c(2,maxpoints))
		yall <- array(NA, c(2,maxpoints))

		xall[1,1:length(km[,2])] <- km[,2]
		yall[1,1:length(km[,1])] <- km[,1]

		xall[2,1:length(bv[,2])] <- bv[,2]
		yall[2,1:length(bv[,1])] <- bv[,1]
		...
		ldesc[1] <- sprintf("KM for %s", classes[classnum])
		lty[1] = 1
		lwd[1] = 2.5

		ldesc[2] <- sprintf("ES for %s", classes[classnum])
		lty[2] = 1
		lwd[2] = 2.5

		legend("bottomright",ldesc,col=colors,lty=lty,lwd=lwd)
	}
}
...
mplus.plot.survival.sample.vs.estimated.logcumulative <- function(file,survvar=1,classnum) {
	...
	if (missing(classnum)) {
		...
		lwd <- array(0,c(2*dims[1]))
		for (i in c(1:dims[1])) {
			ldesc[2*(i-1)+1] <- sprintf("LC for %s", classes[i])
			lty[2*(i-1)+1] = 1
			lwd[2*(i-1)+1] = 2.5

			ldesc[2*i] <- sprintf("ELC for %s", classes[i])
			lty[2*i] = 1
			lwd[2*i] = 2.5
		}
		legend("top",ldesc,col=colors,lty=lty,lwd=lwd)
		...
		npoints <- array(0, c(2))
		slc <- mplus.compute.survival.sample.logcumulative.values(file,surv_idx,classnum)
		npoints[1] <- length(slc[,1])
		elc <- mplus.compute.survival.estimated.logcumulative.values(file,surv_idx,classnum=classnum)
		npoints[2] <- length(elc[,1])
		maxpoints <- max(npoints)

		xall <- array(NA, c(2,maxpoints))
		yall <- array(NA, c(2,maxpoints))

		xall[1,1:length(slc[,2])] <- slc[,2]
		yall[1,1:length(slc[,1])] <- slc[,1]

		xall[2,1:length(elc[,2])] <- elc[,2]
		yall[2,1:length(elc[,1])] <- elc[,1]
		...
		ldesc[1] <- sprintf("LC for %s", classes[classnum])
		lty[1] = 1
		lwd[1] = 2.5

		ldesc[2] <- sprintf("ELC for %s", classes[classnum])
		lty[2] = 1
		lwd[2] = 2.5
...
#
mplus.list.bootstrap.parameters <- function(file,quiet=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
mplus.plot.eigenvalues <-function(file) {
	...
	if (("parallel_average" %in% names(gh5$efa))) {
		x <- array(c(1:length(eigen)),c(length(eigen),3))
		y <- array(0, c(length(eigen),3))
		ldesc <- array(0,c(3))
		lty <- array(0,c(3))
		lpch <- array(0,c(3))
		lcol <- array(0,c(3))

		pa <- mplus.get.group.dataset(file, 'efa', 'parallel_average')
		pci <- mplus.get.group.dataset(file, 'efa', 'parallel_ci')

		y[,1] <- eigen
		ldesc[1] <- c('Sample Eigenvalues')
		lty[1] <- 1
		lpch[1] <- 1
		lcol[1] <- c('red')

		y[,2] <- pa
		ldesc[2] <- c('Parallel Analysis Eigenvalues')
		lty[2] <- 2
		lpch[2] <- 0
		lcol[2] <- c('black')

		y[,3] <- pci
		ldesc[3] <- c('Parallel Analysis 95th Percentile')
		lty[3] <- 4
		lpch[3] <- 2
		lcol[3] <- c('black')

		plot(x,y,xlab='',ylab='',type='n')
		lines(x[,1],y[,1],type='o',pch=lpch[1],lty=lty[1], col=lcol[1])
		lines(x[,2],y[,2],type='o',pch=lpch[2],lty=lty[2], col=lcol[2])
		lines(x[,3],y[,3],type='o',pch=lpch[3],lty=lty[3], col=lcol[3])
		legend('top',col=lcol,pch=lpch,ldesc,lty=lty)
...
##########################################################################
#
# mplus.get.parallel.eigenvalues.average - get average parallel eigenvalues
#
# arguments:
#	file - the quoted name of an existing GH5 file, required
#
# eg. mplus.get.parallel.eigenvalues.average('ex4.1.gh5')
#
mplus.get.parallel.eigenvalues.average <-function(file) {
	if (missing(file)) {
		stop(" - name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (!("efa" %in% names(gh5))) {
		stop("- requires EFA information\n\nSpecify TYPE=EFA and PARALLEL in Mplus to get parallel eigenvalues.\n")
	}

	if (!("parallel_average" %in% names(gh5$efa))) {
		stop("- requires PARALLEL information\n\nSpecify PARALLEL in Mplus to get parallel eigenvalues.\n")
	}

	eigen <- mplus.get.group.dataset(file, 'efa', 'parallel_average')
...
##########################################################################
#
# mplus.get.parallel.eigenvalues.ci - get CI for the parallel eigenvalues
#
# arguments:
#	file - the quoted name of an existing GH5 file, required
#
# eg. mplus.get.parallel.eigenvalues.ci('ex4.1.gh5')
#
mplus.get.parallel.eigenvalues.ci <-function(file) {
	if (missing(file)) {
		stop(" - name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	eigen <- mplus.get.group.dataset(file, 'efa', 'parallel_ci')
...
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.plot.histogram.timepoints('ex8.1.gh5')
#
mplus.plot.histogram.timepoints <- function(file,binlength=1) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	ntimepts <- mplus.get.timeseries.timepoints(file)
	...
	idnums <- mplus.list.timeseries.idnums(file, TRUE)

	if (mplus.check.group.attribute(file, 'individual_data', 'intervals')) {
		maxtime <- max(ntimepts)
		xx <- array(0,maxtime)

		# want to plot based on the original values of the TINTERVAL variable
		#var <- c('_NEWTIME')
		#index <- pmatch(var, var_names, nomatch=0)
		if (num_clusters == 1) {
			index <- length(var_names) - 3
		} else {
			index <- length(var_names) - 4
		}

		clusters <- gh5$individual_data$raw_data[cluster_index,]
		alldata <- gh5$individual_data$raw_data[index,]
		alldata[alldata == 999] <- NA
		df <- data.frame(alldata, clusters)

		for (s in c(1:length(idnums))) {
			dff2 <- df[ which(df$clusters==idnums[s]), 'alldata']
			for (i in c(1:length(dff2))) {
				if (!is.na(dff2[i])) {
					xx[i] <- xx[i] + 1
				}
			}
		}
	} else if (mplus.check.group.attribute(file, 'individual_data', 'continuoustime')) {
		timedf <- mplus.get.timeseries.continuous.timepoints(file)

		# find the min and max time from all the subjects
		mintime <- NA
		maxtime <- 0
		for (s in c(1:length(idnums))) {
			xt <- timedf[ which(timedf$clusters==idnums[s]), 'ctime' ]
			curtime <- xt[length(xt)] / binlength;
			if (maxtime < curtime) {
				maxtime <- curtime
			}
			if (is.na(mintime) || xt[1] < mintime) {
				mintime <- xt[1]
			}
		}
		...
		for (s in c(1:length(idnums))) {
			xt <- timedf[ which(timedf$clusters==idnums[s]), 'ctime']
			for (t in c(1:length(xt))) {
				timeindex <- min(max(1, ((xt[t] - mintime) / binlength + 1)), maxtime )
				xx[timeindex] <- xx[timeindex] + 1
			}
		}
	} else {
		maxtime <- max(ntimepts)
		...
		for (s in c(1:length(ntimepts))) {
			for (i in c(1:ntimepts[s])) {
				xx[i] <- xx[i] + 1
			}
		}
	}

	cstr <- sprintf("Histogram of subjects per timepoint")
	barplot(xx,col="red",main=cstr,xlab="Time point",ylab="Subjects",names.arg=seq(1,maxtime))
}
...
#	idnum - the id number
#
# eg. mplus.plot.time.intervals('ex8.1.gh5',1)
#
mplus.plot.time.intervals <- function(file,idnum) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if ( !(mplus.check.group.attribute(file, 'individual_data', 'intervals'))) {
		stop("- requires time intervals.\n\nModel must use the TINTERVAL option.")
	}

	# get the variable names from individual_data and lookup the indices
	intervals <- mplus.get.timeseries.true.intervals(file, idnum)
	computed <- mplus.get.timeseries.computed.intervals(file, idnum)

	x1 <- array(NA,length(intervals))
	x2 <- array(NA,length(intervals))
	y1 <- array(NA,length(intervals))
	y2 <- array(NA,length(intervals))
	...
	plot(x1,y1,xlab="Time point",ylab="Time value",type="n",ylim=c(0,max(y1,y2,na.rm=TRUE)))
	lines(x1,y1,type='o',pch=21, col="red")
	lines(x2,y2,type='o',pch=22, col="blue")

	#legend("topleft", legend = c("Observed time", "New time midpoint"), col = c("red", "blue"), pch = c(21, 22))
	legend("topleft", c("Observed time", "New time midpoint"), col=c("red", "blue"), pch=c(21,22), lty=c(1,1), lwd=c(1,1))
}

##########################################################################
#
# mplus.plot.timeseries.observed
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	v - name of variable to plot
...
#
mplus.plot.timeseries.observed <- function(file,v,idnum,binlength=1) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (is.numeric(v)) {
		varlist <- mplus.list.timeseries.variables(file, TRUE)
		if (v < 1 || v > length(varlist)) {
			cstr <- paste(c("Index of time series variable out of range:"),v,"\n")
			stop(cstr)
		}
		var <- varlist[v]
		index <- pmatch(var, var_names, nomatch=0)
	...
	if ( !(mplus.check.group.attribute(file, 'individual_data', 'cluster')) ) {
		cstr <- sprintf("Time series plot of %s", var_names[index])
		yy <- mplus.get.timeseries.data(file, var)
		xx <- 1:c(length(yy))
	} else if (missing(idnum)) {
		num_clusters <- mplus.get.group.attribute(file, 'individual_data', 'cluster')
		...
		if (iscontinuoustime) {
			...
			xx <- array(0, c(maxtime))
			xc <- array(0, c(maxtime))
			yy <- array(0,maxtime)
			yc <- array(0,maxtime)

			# set up the array for the low/high values of each bin
			lowrange <- array(0, c(maxtime))
			highrange <- array(0, c(maxtime))
			for (i in c(1:maxtime)) {
				if (i==1) {
					lowrange[i] = mintime
				} else {
					lowrange[i] = highrange[i-1]
				}
				highrange[i] = lowrange[i] + binlength
				xx[i] <- (highrange[i] + lowrange[i]) / 2.0
			}

			# sort the times into bins
			for (s in c(1:length(idnums))) {
				xt <- timedf[ which(timedf$clusters==idnums[s]), 'ctime' ]
				yt <- df[ which(df$clusters==idnums[s]), 'alldata']
				for (t in c(1:length(xt))) {
					timeindex <- min(max(1, ((xt[t] - mintime) / binlength + 1)), maxtime )
					yy[timeindex] <- yy[timeindex] + yt[t]
					yc[timeindex] <- yc[timeindex] + 1
				}
			}
		} else {
			ntimepts <- mplus.get.timeseries.timepoints(file)
			...
			for (s in c(1:length(idnums))) {
				df2 <- df[ which(df$clusters==idnums[s]), ]
				colnames(df2) <- c('xx')
				for (i in c(1:length(df2$xx))) {
					if (!is.na(df2$xx[i])) {
						yy[i] <- yy[i] + df2$xx[i]
						yc[i] <- yc[i] + 1
					}
		...
		cstr <- sprintf("Time series plot of %s, Subject = %d", var_names[index], idnum)
		yy <- mplus.get.timeseries.data(file, var, idnum)
		if (iscontinuoustime) {
			xx <- mplus.get.timeseries.continuous.timepoints(file, idnum)
		} else {
			xx <- 1:c(length(yy))
		}
	}
...
mplus.get.timeseries.data <- function(file,v,idnum) {
	...
	if (missing(idnum)) {
		stop("- requires the subject ID.\n\nUse mplus.list.timeseries.idnums() to get the list of subject ID numbers.")
		# idnums <- mplus.list.timeseries.idnums(file, TRUE)
		# df2 <- data.frame(col.names=idnums)
		# for (i in c(1:length(idnums))) {
		#   df2$idnums[i] <- df[ which(df$clusters==idnums[i]), ]
		# }
	} else {
		df2 <- df[ which(df$clusters==idnum), ]
		if (length(df2$xx) == 0) {
			cstr <- sprintf("- unknown id number (%d).\n\nUse mplus.list.timeseries.idnums('%s') to get the list of id numbers.", idnum, file)
			stop(cstr)
		}
	}
...
##########################################################################
#
# mplus.get.timeseries.true.intervals
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	idnum - the id number
#
# eg. mplus.get.timeseries.true.intervals('ex8.1.gh5',1)
#
mplus.get.timeseries.true.intervals <- function(file,idnum) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	# We don't need to make the distinction between TWOLEVEL and CROSSCLASSIFIED
	# because the second cluster variable in CROSSCLASSIFIED is the extra value
	# saved for TWOLEVEL (along with original and computed time values).
	#num_clusters <- mplus.get.group.attribute(file, 'individual_data', 'cluster')

	index <- length(var_names) - 2
	if (mplus.check.group.attribute(file, 'individual_data', 'idvar')) {
		index <- index - 1
	}
	...
	num_clusters <- mplus.get.group.attribute(file, 'individual_data', 'cluster')
	index <- index - as.numeric(num_clusters)
...
##########################################################################
#
# mplus.get.timeseries.computed.intervals
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	idnum - the id number
#
# eg. mplus.get.timeseries.computed.intervals('ex8.1.gh5',1)
#
mplus.get.timeseries.computed.intervals <- function(file,idnum) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
##########################################################################
#
# mplus.get.timeseries.timepoints - return the number of timepoints for all ids
#
# arguments:
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.get.timeseries.timepoints('ex8.1.gh5')
#
mplus.get.timeseries.timepoints <- function(file) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	idnums <- mplus.list.timeseries.idnums(file,TRUE)
	ntime <- array(0, c(length(idnums)))

	for (i in c(1:length(idnums))) {
		ntime[i] <- length(df[ which(df$clusters==idnums[i]), ])
	}
...
##########################################################################
#
# mplus.get.timeseries.continuous.timepoints - return the continuous timepoints for all ids, or certain id
#
# arguments:
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.get.timeseries.continuous.timepoints('ex8.1.gh5')
#
mplus.get.timeseries.continuous.timepoints <- function(file, idnum) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	# check if timeseries information exists
	if ( !(mplus.check.group.attribute(file, 'individual_data', 'continuoustime'))) {
		stop("- requires continuous time information.\n\nThe CTIME option in the VARIABLE command is required.")
	}
	...
	ctime <- gh5$individual_data$raw_data[index,]
...
#
mplus.list.timeseries.variables <- function(file, quiet=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	for (i in c(1:length(timeseries))) {
		if (timeseries[i] == 1) {
			...
			vnames <- append(vnames, var_names[i])
			if (!quiet) {
				cat(cstr,sep="\n")
			}
		}
...
#
mplus.list.timeseries.idnums <- function(file, quiet = FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
######################################################################################################
# Functions for MEASUREMENT PARAMETER  plots
######################################################################################################

##########################################################################
#
# mplus.list.measurement.parameters - list the measurement parameters in the model
#
# arguments: none
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.list.measurement.parameters('ex8.1.gh5')
#
mplus.list.measurement.parameters <- function(file, quiet = FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	# check if measurement parameter data exists
	if ( !("measurement_parameters" %in% names(gh5)) ) {
		stop("- requires measurement_parameters.\n\nModel does not have measurement parameter data.")
	}

	if (!quiet) {
		cat(c("\nList of measurement parameters to use in the following functions:\n"))
		cat(c(" - mplus.plot.measurement.parameter\n"))

		cat(c("\nParameters:\n"))
	}

	var_names <- gh5$measurement_parameters$statements
	var_names <- gsub("(^\\s+|\\s+$)", "", var_names, perl=TRUE)
	var_names <- toupper(var_names)
	if (!quiet) {
		for (i in c(1:length(var_names))) {
			cstr <- sprintf("[%d] %s", i, var_names[i])
			cat(cstr,sep="\n")
		}
	}
...
##########################################################################
#
# mplus.get.measurement.parameter.values - get the measurement parameter values in the model
#
# arguments: none
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.get.measurement.parameter.values('ex8.1.gh5')
#
mplus.get.measurement.parameter.values <- function(file, parameter=1) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (is.character(parameter)) {
		param_idx <- pmatch(parameter, var_names, nomatch=0)
		if (param_idx == 0) {
			cstr <- paste("- unknown parameter:  ",parameter,"\nUse mplus.list.measurement.parameters for a list of parameters.\n")
			stop(cstr)
		}
	} else {
		if (parameter < 0 || parameter > length(var_names)) {
			cstr <- paste("- parameter index is out of range:  ",parameter,"\nArgument should be between 1 and ", length(var_names),"\n")
			stop(cstr)
		}
		param_idx <- parameter
	}

	gh5$measurement_parameters$parameters[param_idx,]
}


##########################################################################
#
# mplus.plot.measurement.parameter - plot the measurement parameter in the model
#
# arguments: none
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.plot.measurement.parameter('ex8.1.gh5')
#
mplus.plot.measurement.parameter <- function(file, parameter=1,showhist=FALSE,bins=10) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (is.character(parameter)) {
		...
		param_idx <- parameter
		param_label <- var_names[param_idx]
	}

	cstr <- paste("Measurement invariance plot for",param_label)
	yy <- gh5$measurement_parameters$parameters[param_idx,]

	if (showhist) {
		hist(yy,breaks=seq(min(yy, na.rm=TRUE),max(yy, na.rm=TRUE),length=bins+1),col="red",main=cstr,xlab=param_label)
	} else {
		dims <- attr(gh5$measurement_parameters$parameters, 'dim')
		xx <- seq(1,dims[2])

		plot(xx,yy,xlab="Group",ylab="Estimate",main=cstr,type='n')

		if (!is.null(gh5$measurement_parameters$invariance)) {
			x1 <- seq(1,dims[2])
			y1 <- array(gh5$measurement_parameters$invariance[param_idx], c(dims[2]))
			lines(x1, y1,col='black')

			cols <- array('red', c(dims[2]))
			ginvar <- gh5$measurement_parameters$group_invariance[param_idx,]
			for (i in c(1:dims[2])) {
				if (ginvar[i] > 0) {
					cols[ginvar[i]] <- 'green'
				}
			}
			points(xx, yy,pch=15, col=cols)
		} else {
...
##########################################################################
#
# mplus.get.estimated.cluster.variables - get the variables with estimated cluster means/variances in the model
#
# arguments: none
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.get.estimated.cluster.variables('ex8.1.gh5')
#
mplus.get.estimated.cluster.variables <- function(file, observed=TRUE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	# check if means_and_variances_data exists
	if ( !("means_and_variances_data" %in% names(gh5)) ) {
		stop("- requires means_and_variances_data.\n\nModel does not have means and variances data.")
	}

	if (observed) {
		if ( !("y_cluster_means" %in% names(gh5$means_and_variances_data)) ) {
			stop("- requires y_cluster_means.\n\nModel does not have any observed variables with estimated clustered means.")
		}
		mean_vars <- mplus.get.group.attribute(file,'means_and_variances_data/y_cluster_means','variables')
	} else {
		if ( !("e_cluster_means" %in% names(gh5$means_and_variances_data)) ) {
			stop("- requires e_cluster_means.\n\nModel does not have any latent variables with estimated clustered means.")
		}
		mean_vars <- mplus.get.group.attribute(file,'means_and_variances_data/e_cluster_means','variables')
	}
	mean_vars
}
...
##########################################################################
#
# mplus.get.estimated.cluster.values - get the estimated cluster values in the model
#
# arguments: none
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.get.estimated.cluster.values('ex8.1.gh5')
#
mplus.get.estimated.cluster.values <- function(file,id=NULL,var=NULL,groupidx=1,vtype=1) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (!is.null(id)) {
		ids <- mplus.list.cluster.idnums(file)
		idindex <- pmatch(id, ids, nomatch=0)
		if (idindex == 0) {
			cstr <- paste(c("Unknown cluster ID:"),id,"\n")
			stop(cstr)
		}
	}

	if (vtype == 1) {
		if ( !("y_cluster_means" %in% names(gh5$means_and_variances_data)) && !("e_cluster_means" %in% names(gh5$means_and_variances_data)) ) {
			stop("- requires y_cluster_means or e_cluster_means.\n\nModel does not have estimated clustered means.")
		}
		if (!is.null(var)) {
			found <- FALSE
			if ("y_cluster_means" %in% names(gh5$means_and_variances_data)) {
				mean_vars <- mplus.get.group.attribute(file,'means_and_variances_data/y_cluster_means','variables')
				var <- toupper(var)
				varindex <- pmatch(var, mean_vars, nomatch=0)
				if (varindex > 0) {
					found <- TRUE
					if (!is.null(id)) {
						return(gh5$means_and_variances_data$y_cluster_means$values[varindex,idindex,groupidx])
					} else {
						return(gh5$means_and_variances_data$y_cluster_means$values[varindex,,groupidx])
					}
				}
			}
			if (!found && "e_cluster_means" %in% names(gh5$means_and_variances_data)) {
				mean_vars <- mplus.get.group.attribute(file,'means_and_variances_data/e_cluster_means','variables')
				var <- toupper(var)
				varindex <- pmatch(var, mean_vars, nomatch=0)
				if (varindex > 0) {
					found <- TRUE
					if (!is.null(id)) {
						return(gh5$means_and_variances_data$e_cluster_means$values[varindex,idindex,groupidx])
					} else {
						return(gh5$means_and_variances_data$e_cluster_means$values[varindex,,groupidx])
					}
				} else {
					cstr <- paste(c("1Unknown variable or variable does not have estimated cluster means:"),var,"\n")
					stop(cstr)
				}
			} else if (!found) {
				cstr <- paste(c("2Unknown variable or variable does not have estimated cluster means:"),var,"\n")
				stop(cstr)
			}
		} else { # var is null
			if (!is.null(id)) {
				return(gh5$means_and_variances_data$e_cluster_means$values[,idindex,groupidx])
			} else {
				return(gh5$means_and_variances_data$e_cluster_means$values[,,groupidx])
			}
		}
	} else { # vtype != 1 (variance)
		if ( !("y_cluster_variances" %in% names(gh5$means_and_variances_data)) && !("e_cluster_variances" %in% names(gh5$means_and_variances_data)) ) {
			stop("- requires y_cluster_variances or e_cluster_variances.\n\nModel does not have estimated clustered variances.")
		}
		if (!is.null(var)) {
			found <- FALSE
			if ("y_cluster_variances" %in% names(gh5$means_and_variances_data)) {
				mean_vars <- mplus.get.group.attribute(file,'means_and_variances_data/y_cluster_variances','variables')
				var <- toupper(var)
				varindex <- pmatch(var, mean_vars, nomatch=0)
				if (varindex > 0) {
					found <- TRUE
					if (!is.null(id)) {
						return(gh5$means_and_variances_data$y_cluster_variances$values[varindex,idindex,groupidx])
					} else {
						return(gh5$means_and_variances_data$y_cluster_variances$values[varindex,,groupidx])
					}
				}
			}
			if (!found && "e_cluster_variances" %in% names(gh5$means_and_variances_data)) {
				mean_vars <- mplus.get.group.attribute(file,'means_and_variances_data/e_cluster_variances','variables')
				var <- toupper(var)
				varindex <- pmatch(var, mean_vars, nomatch=0)
				if (varindex > 0) {
					found <- TRUE
					if (!is.null(id)) {
						return(gh5$means_and_variances_data$e_cluster_variances$values[varindex,idindex,groupidx])
					} else {
						return(gh5$means_and_variances_data$e_cluster_variances$values[varindex,,groupidx])
					}
				} else {
					cstr <- paste(c("Unknown variable or variable does not have estimated cluster variances:"),var,"\n")
					stop(cstr)
				}
			} else if (!found) {
				cstr <- paste(c("Unknown variable or variable does not have estimated cluster variances:"),var,"\n")
			...
			if (!is.null(id)) {
				return(h5$means_and_variances_data$e_cluster_variances$values[,idindex,groupidx])
			} else {
				return(gh5$means_and_variances_data$e_cluster_variances$values[,,groupidx])
			}
		}
	}
}
...
##########################################################################
#
# mplus.get.estimated.cluster.variances - get the estimated cluster variances in the model
#
# arguments: none
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.get.estimated.cluster.variances('ex8.1.gh5')
#
mplus.get.estimated.cluster.variances <- function(file,id=NULL,var=NULL,groupidx=1,observed=TRUE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (observed) {
		if ( !("y_cluster_variances" %in% names(gh5$means_and_variances_data)) ) {
			stop("- requires y_cluster_variances.\n\nModel does not have estimated clustered variances for the observed variables.")
		}
		if (!is.null(var)) {
			mean_vars <- mplus.get.group.attribute(file,'means_and_variances_data/y_cluster_variances','variables')
			var <- toupper(var)
			...
			if (!is.null(id)) {
				gh5$means_and_variances_data$y_cluster_variances$values[varindex,idindex,groupidx]
			} else {
				gh5$means_and_variances_data$y_cluster_variances$values[varindex,,groupidx]
			}
		} else {
			if (!is.null(id)) {
				gh5$means_and_variances_data$y_cluster_variances$values[,idindex,groupidx]
			} else {
				gh5$means_and_variances_data$y_cluster_variances$values[,,groupidx]
			}
		}
	} else {
		if ( !("e_cluster_variances" %in% names(gh5$means_and_variances_data)) ) {
			stop("- requires e_cluster_variances.\n\nModel does not have estimated clustered variances for the latent variables.")
		}
		if (!is.null(var)) {
			mean_vars <- mplus.get.group.attribute(file,'means_and_variances_data/e_cluster_variances','variables')
			var <- toupper(var)
			...
			if (!is.null(id)) {
				gh5$means_and_variances_data$e_cluster_variances$values[varindex,idindex,groupidx]
			} else {
				gh5$means_and_variances_data$e_cluster_variances$values[varindex,,groupidx]
			}
		} else {
			if (!is.null(id)) {
				gh5$means_and_variances_data$e_cluster_variances$values[,idindex,groupidx]
			} else {
				gh5$means_and_variances_data$e_cluster_variances$values[,,groupidx]
			}
		}
	}
}
...
##########################################################################
#
# mplus.get.average.within.cluster - get the within-cluster average of variable
#
# arguments: none
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.get.average.within.cluster('ex8.1.gh5')
#
mplus.get.values.over.within.cluster <- function(file,id=NULL,var=NULL,groupidx=1,clusvar,vtype=1) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (!is.null(var)) {
		var <- toupper(var)
		varindex <- pmatch(var, var_names, nomatch=0)
		if (varindex == 0) {
			cstr <- paste(c("Unknown variable:"),var,"\n")
			stop(cstr)
		}
		if (var_levels[varindex] > 1) {
			cstr <- paste(c("Variable belongs to a level higher than 1 and has no within parts:"),var,"\n")
			stop(cstr)
		}
	}
	...
	clusterids <- gh5$individual_data$raw_data[clusindex,]
	...
	if (!is.null(id)) {
		...
		xx <- gh5$individual_data$raw_data[varindex,which(clusterids==id)]
		xx[xx == 999] <- NA
		if (vtype == 1) {
			values <- mean(xx,na.rm=TRUE)
		} else {
			n <- length(xx[!is.na(xx)])
			values <- var(xx,na.rm=TRUE) * ((n-1)/n)
		}
	} else {
		values <- array(NA,c(length(ids)))
		for (i in c(1:length(ids))) {
			xx <- gh5$individual_data$raw_data[varindex,which(clusterids==ids[i])]
			xx[xx == 999] <- NA
			if (vtype == 1) {
				values[i] <- mean(xx,na.rm=TRUE)
			} else {
				n <- length(xx[!is.na(xx)])
				values[i] <- var(xx,na.rm=TRUE) * ((n-1)/n)
			}
		}
	}
	values
...
##########################################################################
#
# mplus.has.significance - check to see if variable has significance
#
# arguments: none
#	file - the quoted name of an existing GH5 file
#	variable - the variable name or variable index
#
# eg. mplus.has.significance('ex8.1.gh5',variable)
#
mplus.has.significance <- function(file,var) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (!mplus.check.group.attribute(file, 'individual_data', 'significance')) {
		return(FALSE)
	}

	# get the variable names from individual_data and lookup the indices
	sigs <- mplus.get.group.attribute(file, 'individual_data', 'significance')
	...
	if (varindex == 0 || varindex > length(var_names)) {
		cstr <- paste(c("Unknown variable:"),var,"\n")
		stop(cstr)
	}
...
##########################################################################
#
# mplus.get.significance - get the significance data for the given variable
#
# arguments: none
#	file - the quoted name of an existing GH5 file
#	variable - the variable name or variable index
#
# eg. mplus.get.significance('ex8.1.gh5',variable)
#
mplus.get.significance <- function(file,var,between=FALSE,clusvar=NULL) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (sigs[varindex] == 0) {
		cstr <- paste(c("variable does not have significance data:  "),var_names[varindex],"\n")
		stop(cstr)
	}
	sigindex <- 0
	for (i in c(1:varindex)) {
		if (sigs[i] == 1) {
			sigindex <- sigindex + 1
		}
	}
	...
	if (between) {
		...
		xx <- array(NA,c(1,length(ids)))
		for (i in c(1:length(ids))) {
			xx[,i] <- gh5$individual_data$significance_data[sigindex,which(vids == ids[i])][1]
		}
	} else {
		xx <- gh5$individual_data$significance_data[sigindex,]
	}
...
######################################################################################################
# Functions for AUTOREGRESSIVE CURVES
######################################################################################################

#========================================================================
#
# mplus.list.autoregressive.curve.parameters - list the parameters for autoregressive curves
#
# arguments:
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.list.autoregressive.curve.parameters('ex8.1.gh5')
#
mplus.list.autoregressive.curve.parameters <- function(file, quiet=FALSE) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	# check if drift_curve_data exists
	if ( !("drift_curve_data" %in% names(gh5)) ) {
		stop("- requires drift_curve_data.\n\nUse TYPE=PLOT2 or TYPE=PLOT3 with the DRIFT option.")
	}

	if (!quiet) {
		cat(c("\nList of parameters to use in the following functions:\n"))
		cat(c(" - mplus.plot.autoregressive.curves\n"))
		cat(c(" - mplus.get.autoregressive.estimates\n"))
		cat(c(" - mplus.get.autoregressive.lowerci\n"))
		cat(c(" - mplus.get.autoregressive.upperci\n"))
		cat(c(" - mplus.get.autoregressive.xvalues\n"))

		cat(c("\nParameters for autoregressive curves:\n"))
	}

	# get the parameter statements from drift_curve_data and lookup the indices
	statements <- mplus.get.group.attribute(file, 'drift_curve_data', 'labels')
	statements <- gsub("(^\\s+|\\s+$)", "", statements, perl=TRUE)
...
#========================================================================
#
# mplus.get.autoregressive.estimates - get the estimates for the given autoregressive parameter
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	label - the quoted label or index of the label in the list
#
# eg. mplus.get.autoregressive.estimates('ex8.1.gh5','indirect')
#
mplus.get.autoregressive.estimates <- function(file,label=1) {
	if (missing(file)) {
		stop("- - name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	# check if drift curve data exists
	if ( !("drift_curve_data" %in% names(gh5)) ) {
		stop(" - requires drift_curve_data\n\nUse TYPE=PLOT2 or TYPE=PLOT3 and the DRIFT option.")
	}

	if (is.character(label)) {
		labels <- mplus.get.group.attribute(file,'drift_curve_data','labels')
		labels <- gsub("(^\\s+|\\s+$)", "", labels, perl=TRUE)
		labels <- tolower(labels)
		label <- tolower(label)
		loopidx <- pmatch(label, labels, nomatch=0)
		...
		if (loopidx == 0) {
			cstr <- paste(c(" - unknown label:"),label,"\n\nUse mplus.list.autoregressive.curve.parameters to see the list of labels.\n")
			stop(cstr)
		}
	} else {
		# get the dimensions of the estimates dataset
		# first dimension is the number of loop labels
		# second dimension is the number of x points
		dims <- attr(gh5$drift_curve_data$estimates,'dim')
	...
	gh5$drift_curve_data$estimates[loopidx,]
}


#========================================================================
#
# mplus.get.autoregressive.lowerci - get the lower CI values for the given autoregressive parameter
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	label - the quoted label or index of the label in the list
#
# eg. mplus.get.autoregressive.lowerci('ex8.1.gh5','indirect')
#
mplus.get.autoregressive.lowerci <- function(file,label=1) {
	if (missing(file)) {
		stop("- - name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	gh5$drift_curve_data$lowerci[loopidx,]
}


#========================================================================
#
# mplus.get.autoregressive.upperci - get the upper CI values for the given autoregressive parameter
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	label - the quoted label or index of the label in the list
#
# eg. mplus.get.autoregressive.upperci('ex8.1.gh5','indirect')
#
mplus.get.autoregressive.upperci <- function(file,label=1) {
	if (missing(file)) {
		stop("- - name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	gh5$drift_curve_data$upperci[loopidx,]
}

#========================================================================
#
# mplus.get.autoregressive.timevalues - get the time points for the autoregressive curve
#
# arguments:
#	file - the quoted name of an existing GH5 file
#
# eg. mplus.get.autoregressive.timevalues('ex8.1.gh5')
#
mplus.get.autoregressive.timevalues <- function(file) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	gh5$drift_curve_data$timevalues
}

#========================================================================
#
# mplus.plot.autoregressive.curve - plot the parameter with autoregressive curve
#
# arguments:
#	file - the quoted name of an existing GH5 file
#	label - the quoted label or index of the label in the list
#	showgrid - option to turn off grid lines, default is to show grid
#
# eg. mplus.plot.autoregressive.curve('ex8.1.gh5',1)
#
mplus.plot.autoregressive.curve <- function(file,label=1,showgrid=TRUE,ylim,linecolors,linetype,noci=FALSE,xlabel,ylabel) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	# get the dimensions of the estimates dataset
	# first dimension is the number of autoregressive labels
	# second dimension is the number of x points
	props <- mplus.get.group.attribute(file,'drift_curve_data','properties')
	...
	if (length(label) > 1) {
		...
		for (r in c(1:num_loop)) {
			loopidx <- loopindices[r]
			if (noci) {
				xx[(r-1)+1,] <- mplus.get.autoregressive.timevalues(file)

				yy[(r-1)+1,] <- mplus.get.autoregressive.estimates(file,loopidx)
			} else {
				xx[3*(r-1)+1,] <- mplus.get.autoregressive.timevalues(file)
				xx[3*(r-1)+2,] <- mplus.get.autoregressive.timevalues(file)
				xx[3*(r-1)+3,] <- mplus.get.autoregressive.timevalues(file)

				yy[3*(r-1)+1,] <- mplus.get.autoregressive.estimates(file,loopidx)
				yy[3*(r-1)+2,] <- mplus.get.autoregressive.lowerci(file,loopidx)
				yy[3*(r-1)+3,] <- mplus.get.autoregressive.upperci(file,loopidx)
			}
		}

		# plot the loop
		cstr <- paste("Autoregressive curves")
		if (missing(ylim)) {
			ylim <- c(min(yy),max(yy))
		}
	...
	xx <- array(0,c(3,props[2]))
	xx[1,] <- mplus.get.autoregressive.timevalues(file)
	xx[2,] <- mplus.get.autoregressive.timevalues(file)
	xx[3,] <- mplus.get.autoregressive.timevalues(file)

	yy <- array(0,c(3,props[2]))
	yy[1,] <- mplus.get.autoregressive.estimates(file,loopidx)
	yy[2,] <- mplus.get.autoregressive.lowerci(file,loopidx)
	yy[3,] <- mplus.get.autoregressive.upperci(file,loopidx)

	# plot the loop
	cstr <- paste("Autoregressive curve for",labels[loopidx])
...
######################################################################################################
# New functions for RPlotsUI
######################################################################################################
mplus.get.available.plots <- function(file) {
	...
	plotlist <- mplus.get.group.attribute(file, '/', 'graph_list')
	plotlist <- gsub("(.+)\\|.*\\|.*","\\1", plotlist)
	plotlist
}

mplus.get.num.processes <- function(file) {
	...
	num <- mplus.get.group.attribute(file, '/process_data', 'num_processes')
	num
}

mplus.get.num.model_groups <- function(file) {
	if ( !(file.exists(file))) {
		cstr <- paste("- file does not exist:",file,"\n")
		stop(cstr)
	}
...
mplus.get.num.data_groups <- function(file) {
	if ( !(file.exists(file))) {
		cstr <- paste("- file does not exist:",file,"\n")
		stop(cstr)
	}

	num_groups <- length(mplus.get.file.dataset(file,'data_group_labels'))

	num_groups
}

mplus.get.model_group_labels <- function(file) {
	if ( !(file.exists(file))) {
		cstr <- paste("- file does not exist:",file,"\n")
		stop(cstr)
	}
...
mplusapp.get.probability_variables <- function(file) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	variables <- vector()
	nvars <- length(catvars)
	for (i in c(1:nvars)) {
		if (vartypes[i] >= 0 &&  categories[i] > 0) {
			variables <- append(variables, catvars[i])
		}
	}
...
mplusapp.get.irt_variables <- function(file) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
mplusapp.get.irt_categories <- function(file, uvar) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
mplusapp.has.individual_data <- function(file) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
mplusapp.get.individual.data.size <- function(file) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
mplusapp.get.bayesian.chains <- function (file) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
...
mplusapp.get.variable.list.between.plots <- function(file) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	# check if individual data exists
	if ( !("individual_data" %in% names(gh5)) ) {
		stop("- requires individual data.\n\nUse TYPE=PLOT3 setting in Mplus to save individual data.")
	}
	...
	num_y <- as.integer(mplus.get.group.attribute(file, 'individual_data', 'n_y'))
	if (mplus.check.group.attribute(file, 'individual_data', 'n_aux')) {
		num_aux <- as.integer(mplus.get.group.attribute(file, 'individual_data', 'n_aux'))
	} else {
		num_aux <- 0
	}
	if (mplus.check.group.attribute(file, 'individual_data', 'n_fscores')) {
	...
	if (mplus.check.group.attribute(file, 'individual_data', 'n_obs_fscores')) {
		num_obs_fscores <- as.integer(mplus.get.group.attribute(file, 'individual_data', 'n_obs_fscores'))
	} else {
		num_obs_fscores <- 0
	}
	...
	for (i in c(1:num_y)) {
		if (var_levels[i] > 1) {
			betweenvars <- append(betweenvars, var_names[i])
		} else if (var_levels[i] < 2) {
			if (!grepl("&1", var_names[i], fixed = TRUE)) {
				cstr <- sprintf("%s (average over Within)", var_names[i])
				betweenvars <- append(betweenvars, cstr)
				cstr <- sprintf("%s (variance over Within)", var_names[i])
				betweenvars <- append(betweenvars, cstr)
			}
		}
	}
	for (i in c((num_y+num_aux+1):(num_y+num_aux+num_fscores))) {
		if (var_levels[i] > 1) {
			betweenvars <- append(betweenvars, var_names[i])
		} else if (var_levels[i] < 2) {
			if (!grepl("&1", var_names[i], fixed = TRUE)) {
	...
	for (i in c((num_y+num_aux+num_fscores+1):(num_y+num_aux+num_fscores+num_obs_fscores))) {
		if (var_levels[i] > 1) {
			betweenvars <- append(betweenvars, var_names[i])
		} else if (var_levels[i] < 2) {
			if (!grepl("&1", var_names[i], fixed = TRUE)) {
	...
	if ("means_and_variances_data" %in% names(gh5)) {
		if ("y_cluster_means" %in% names(gh5$means_and_variances_data)) {
			mean_vars <- mplus.get.group.attribute(file,'means_and_variances_data/y_cluster_means','variables')
			mean_vars <- gsub("(^\\s+|\\s+$)", "", mean_vars, perl=TRUE)
			for (i in c(1:length(mean_vars))) {
				varindex <- pmatch(mean_vars[i], var_names, nomatch=0)
				if (varindex != 0 && var_levels[varindex] < 2 && !grepl("&1", mean_vars[i], fixed = TRUE)) {
					cstr <- sprintf("%s (estimated cluster mean)", mean_vars[i])
					betweenvars <- append(betweenvars, cstr)
					cstr <- sprintf("%s (estimated cluster variance)", mean_vars[i])
					betweenvars <- append(betweenvars, cstr)
				}
			}
		}
	...
	betweenvars <- append(betweenvars, var_names[clusindex])
	betweenvars
}

mplusapp.find.between.plots.variable <- function(file,var) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	if (missing(var)) {
		stop("- requires the variable from the list")
	}
	...
	is_data <- array(FALSE, c(length(var)))
	variable_name <- array(c(0), c(length(var)))
	sample_over_within <- array(FALSE, c(length(var)))
	mean_value <- array(FALSE, c(length(var)))
	estimated_cluster_value <- array(FALSE, c(length(var)))
	is_clusterid <- array(FALSE, c(length(var)))

	found <- FALSE
	varindex <- array(0, c(length(var)))
	for (i in c(1:length(var))) {
		varindex[i] <- pmatch(var[i], var_names, nomatch=0)
	}
	for (i in c(1:length(varindex))) {
		if (varindex[i] > 0) {
			variable_name[i] <- var[i]
			is_data[i] <- TRUE
			...
			if (varindex[i] == clusindex) {
				is_clusterid[i] <- TRUE
			}
		}

		if (varindex[i] == 0) {
			if (grepl("(average over Within)", var[i], ignore.case = TRUE)) {
				variable_name[i] <- strsplit(var[i], ' \\(')[[1]][1]
				sample_over_within[i] <- TRUE
				mean_value[i] <- TRUE
			} else if (grepl("(variance over Within)", var[i], ignore.case = TRUE)) {
				variable_name[i] <- strsplit(var[i], ' \\(')[[1]][1]
				sample_over_within[i] <- TRUE
			} else if (grepl("estimated cluster", var[i], ignore.case = TRUE)) {
				if (!("means_and_variances_data" %in% names(gh5))) {
					stop(" - estimated cluster values are not available for this model")
				}
				if (grepl("(estimated cluster mean)", var[i], ignore.case = TRUE)) {
					variable_name[i] <- strsplit(var[i], ' \\(')[[1]][1]
					estimated_cluster_value[i] <- TRUE
					mean_value[i] <- TRUE
				} else if (grepl("(estimated cluster variance)", var[i], ignore.case = TRUE)) {
					variable_name[i] <- strsplit(var[i], ' \\(')[[1]][1]
					estimated_cluster_value[i] <- TRUE
				}
			} else {
				cstr <- paste("- unknown between information:", var[i])
				stop(cstr)
			}
		}
	}

	df <- data.frame(is_data, variable_name, sample_over_within, mean_value, estimated_cluster_value, is_clusterid)
	t(df)
}

mplusapp.variable.has.estimated.values <- function(file,v) {
	if (missing(file)) {
		stop("- name of the GH5 file is required")
	}
	if (!(file.exists(file))) {
	...
	num_y <- as.integer(props[1]) - as.integer(props[3])
